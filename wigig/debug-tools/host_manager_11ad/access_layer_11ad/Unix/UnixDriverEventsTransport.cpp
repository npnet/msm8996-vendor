...
{
    if (driverEventReport.evt_type != DRIVER_EVENT_FW_EVENT)
    {
        return false;
    }
...
void UnixDriverEventsTransport::DriverControlEventsThread(FwStateProvider* fwStateProvider)
{
    ...
    while (!m_stopRunningDriverControlEventsThread.load())
    {
        ...
        if (g_LogConfig.ShouldPrint(LOG_SEV_DEBUG) && driverEvent.evt_type == DRIVER_EVENT_FW_EVENT)
        {
            uint32_t eventId = 0U;
            if (driverEvent.buf_len > sizeof(uint32_t))
            {
                eventId = reinterpret_cast<uint32_t*>(driverEvent.buf)[0];
            }
