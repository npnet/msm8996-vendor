...
void HostConfigurationSection::ResetConfig()
{
    ...
    Ignore3ppFwEventIds = "0x1A0A,0x1A0B";
}
...
PostLogCollectionSection::PostLogCollectionSection(JsonTree& configTreeRoot)
    : ConfigurationSection("PostCollection", configTreeRoot)
...
void PostLogCollectionSection::ResetConfig()
{
    TargetServer = "Not-Configured";
    UserName = "Not-Configured";
    RemotePath = "Not-Configured";
}
...
void PersistentConfiguration::Save()
{
    ...
    if (!FileSystemOsAbstraction::DoesFolderExist(configFolder) &&
        !FileSystemOsAbstraction::CreateFolder(configFolder))
    {
        LOG_ERROR << "Failed to create configuration folder: " << configFolder << std::endl;
    }
