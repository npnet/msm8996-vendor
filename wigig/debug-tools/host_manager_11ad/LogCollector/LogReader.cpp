...
bool LogReader::ComputeLogBufferStartAddress()
{
    ...
    if (!os)
    {
        LOG_ERROR << m_debugLogPrefix << "Cannot compute log buffer start address: " << os.GetStatusMessage() << endl;
        return  false;
    }
    ...
    os = Host::GetHost().GetDeviceManager().Read(m_deviceName, addressToReadLogOffset, logBufferInfoDword);
    if (!os)
    {
