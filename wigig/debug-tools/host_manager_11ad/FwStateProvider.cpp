    ...
    m_fwUcodeErrorsPollerTaskName("fw_error_" + m_device.GetDeviceName())
{
    ...
    {
        // init FW version
        m_device.ReadDeviceFwInfo(m_fwIdentifier, m_uCodeTimestampStartAddress);
...
void FwStateProvider::ReadDeviceFwInfoInternal()
{
    ...
    if (m_device.ReadDeviceFwInfo(fwIdentifier, m_uCodeTimestampStartAddress) && fwIdentifier != m_fwIdentifier)
    {
