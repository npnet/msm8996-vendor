...
#define QDSS_TRACE_CONFIG_FILE_OLD "/data/vendor/wifi/qdss_trace_config.bin"
		   ...
		   bool append)
{
	...
	wsvc_printf_dbg("%s: Start to store data to: %s", __func__, filename);
	...
	fclose(fp);
	wsvc_printf_dbg("%s: End to store data to: %s", __func__, filename);
...
int wlfw_send_qdss_trace_config_download_req(void)
{
	...
	file = fopen(QDSS_TRACE_CONFIG_FILE_NEW, "rb");
	if (!file)
		file = fopen(QDSS_TRACE_CONFIG_FILE_OLD, "rb");
