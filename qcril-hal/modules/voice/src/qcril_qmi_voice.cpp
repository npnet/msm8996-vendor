...
)
{
  ...
  do {
    if (number.size() > 0) {
      ...
      if (getVoiceModule().getFeature(VoiceFeatureType::SUBADDRESS_SUPPORT_AMP)) {
        subAddressPosition = number.find("&");
      }
      if (subAddressPosition == std::string::npos) {
        if (getVoiceModule().getFeature(VoiceFeatureType::SUBADDRESS_SUPPORT) &&
            number[0] != '*') {
          subAddressPosition = number.find("*");
        }
      }
      if (subAddressPosition != std::string::npos) {
        ...
        if (getVoiceModule().getFeature(VoiceFeatureType::SUBADDRESS_IA5_ID_SUPPORT)) {
...
qcril::interfaces::ClipStatus mapProvisionStatusToClipStatus
(
 provision_status_enum_v02 in
)
{
...
void qcril_qmi_voice_last_call_fail_request_timeout_handler(void * param)
{
  ...
  if (qcril_qmi_voice_info.last_call_failure_cause.pending_req)
  {
    ...
    auto pendingMsg = getVoiceModule().getPendingMessageList().extract(
...
)
{
  ...
  if (call_status_ind_ptr && call_info)
  {
    ...
    if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(call_info_entry,
          QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_ALTERNATE_EMERGENCY_CALL))
    {
...
)
{
  ...
  if (data_ptr != NULL)
  {
    ...
    if (call_info != NULL)
    {
      ...
      std::shared_ptr<Message> pendingMsg = getVoiceModule().getPendingMessageList().find(
      ...
      if (pendingMsg == nullptr)
      {
        pendingMsg = getVoiceModule().getPendingMessageList().find(
      ...
      if (pendingMsg != nullptr)
      {
        ...
        {
          getVoiceModule().getPendingMessageList().erase(pendingMsg);
          if (pendingMsg)
          {
...
)
{
  ...
  if( data_ptr )
  {
     ...
     if (speech_codec_ptr->call_id_valid && speech_codec_ptr->speech_codec_valid)
     {
        ...
        if (getVoiceModule().getFeature(VoiceFeatureType::REPORT_SPEECH_CODEC))
        {
          if (call_info_entry)
          {
...
)
{
  ...
  if( ind_data_ptr != NULL )
  {
    ...
    if ( info_rec_ind->ext_display_record_valid )
    {
      ...
      bool is_0x9e_not_treat_as_name = getVoiceModule().getFeature(VoiceFeatureType::IS_0x9E_NOT_TREAT_AS_NAME);
  ...
  if (dial_call_resp != NULL && NULL != call_info_entry)
  {
    ...
    { // default case: STK CC is support by Android framework
       ...
       {  // STK CC session started
            ...
            auto pendingMsg = getVoiceModule().getPendingMessageList().find(req_id);
...
)
{
  ...
  do {
    ...
    {
      qcril_qmi_voice_voip_call_info_set_single_elaboration(call_info_entry,
          QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_ALTERNATE_EMERGENCY_CALL, TRUE);
    }
    ...
    if (!(getVoiceModule().getFeature(VoiceFeatureType::ENCRYPTED_CALLS_SUPPORT)))
    {
...
)
{
  ...
  do {
    ...
    *number_presentation = QCRIL_QMI_VOICE_RIL_PI_UNKNOWN;
    ...
    // ------------------------------------------------------------------------------------------
      if (getVoiceModule().getFeature(VoiceFeatureType::REDIR_PARTY_NUM_SUPPORT) &&
...
)
{
    ...
    do
    {
        ...
        if ( qcril_qmi_voice_voip_call_info_is_single_elaboration_set( call_info_entry,
              QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_STK_CC_EMULATED_OVERLAY )
              &&
        ...
        if (is_ims)
        {
            ...
            if (call_info_entry->elaboration & QCRIL_QMI_VOICE_VOIP_CALLINFO_ELA_RTT_CAP_INFO_VALID)
            {
        ...
        //sip error code
        if (call_info_entry->elaboration & QCRIL_QMI_VOICE_VOIP_CALLINFO_ELA_SIP_ERROR_CODE_VALID)
        {
        ...
        // end_reason_text
        if (is_ims && (call_info_entry->elaboration &
        ...
        payload_ptr->terminating_num_valid[iter] = FALSE;
        if (call_info_entry->elaboration & QCRIL_QMI_VOICE_VOIP_CALLINFO_ELA_TERMINATING_NUM_VALID)
        {
          payload_ptr->terminating_num_valid[iter] = TRUE;
        ...
        if (VOICE_INVALID_CALL_ID != call_info_entry->android_call_id &&        // not a shadow call
                  ...
                  VOICE_INVALID_CALL_ID != call_info_entry->android_call_id )
          {
                     ...
                     (qcril_qmi_voice_call_to_atel(call_info_entry) && !is_ims))
             {
                ...
                call_info_entry->elaboration |=
                      QCRIL_QMI_VOICE_VOIP_CALLINFO_ELA_CALL_ENDED_REPORTED;
...
)
{
    ...
    auto pendingMsg = module.getPendingMessageList().find(
            QcRilRequestCsHangupMessage::get_class_message_id());
    if (pendingMsg != nullptr)
    {
  ...
  if (sups_ind_msg != NULL)
  {
          ...
          (QCRIL_QMI_VOICE_ALPHA_LENGTH_IN_NULL_CASE == sups_ind_msg->alpha_ident.alpha_text_len)))
    {
      if (((!is_stk_cc_in_progress &&
              ...
              getVoiceModule().getFeature(VoiceFeatureType::PROCESS_SUPS_IND)) || // for proactive cases
...
//  qcril_request_resp_params_type ril_response;
  ...
  { // STK CC session started
    ...
    // msg_token_id
    auto pendingMsg = getVoiceModule().getPendingMessageList().find(msg_token_id);
    if (pendingMsg)
    {
      auto msg = std::static_pointer_cast<QcRilRequestMessage>(pendingMsg);
      if (msg->isImsRequest()) {
        ...
        if (call_status_ind->ip_num_info[j].call_id == qmi_call_id)
        {
          ...
          if (need_update_remote_party_details ||
              (!call_status_ind->alpha_id_valid &&
               !(getVoiceModule().getFeature(VoiceFeatureType::IS_ORIGINAL_NUMBER_DISPLAY))))
          {
...
{
  bool feature_fake_audio_up = getVoiceModule().getFeature(VoiceFeatureType::FAKE_AUDIO_UP);
...
)
{
    ...
    auto pendingMsg = getVoiceModule().getPendingMessageList().find(
    ...
    if (pendingMsg)
    {
      ...
      if (msg->getLocalHoldType() == QcRilRequestSetLocalCallHoldMessage::LocalHoldType::HOLD) {
        ...
        if (msg->getPendingMessageState() != PendingMessageState::AWAITING_CALLBACK) {
          getVoiceModule().getPendingMessageList().erase(msg);
          auto respPayload = std::make_shared<QcRilRequestMessageCallbackPayload>(
...
void qcril_qmi_voice_update_ussd_power_opt_buffer(voice_ussd_ind_msg_v02 *ind_data_ptr)
{
  ...
  if (ind_data_ptr)
  {
    qcril_qmi_voice_info.ussd_info_buffer_for_opt.push_back(*ind_data_ptr);
  }
...
void qcril_qmi_voice_notify_ussd_power_opt_to_atel()
{
  ...
  {
    qcril_qmi_voice_ussd_ind_hdlr(&ind, sizeof(voice_ussd_ind_msg_v02));
  }
  qcril_qmi_voice_reset_ussd_power_opt_buffer();
}
