...
void ImsRadioImpl_1_5::sendImsSmsResponse(uint32_t token, RIL_Errno errorCode,
  ...
  if (respCb != nullptr) {
    ...
    V1_5::utils::convertRilImsResponseToHidlResponse(errorCode, tech, bLteOnlyReg,
                                                     statusReport, failureReason);
    imsRadiolog("<", "sendImsSmsResponse: token=" + std::to_string(token) +
                         " msgeRef=" + std::to_string(messageRef) + " statusReport=" +
                         toString(statusReport) + " reason=" + toString(failureReason));
    auto retStatus =
        respCb->sendImsSmsResponse_1_5(token, messageRef, statusReport, failureReason);
    if (!retStatus.isOk()) {
  ...
  if (respCb == nullptr) {
    ...
    if (msg) {
      if (tech == RADIO_TECH_3GPP) {
        ...
        if (V1_0::utils::convertPayloadToCdmaFormat(imsSms.pdu.data(), imsSms.pdu.size(),
                                                    cdmaMsg)) {
...
Return<void> ImsRadioImpl_1_5::acknowledgeSms_1_5(int32_t token, uint32_t messageRef,
  ...
  auto msg = std::make_shared<RilRequestAckImsSmsMessage>(
      getContext(token), messageRef, V1_5::utils::convertHidlToRilSmsAckResult(smsResult));
  if (msg) {
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        ([token]([[maybe_unused]] std::shared_ptr<Message> msg,
          ...
          (void)token;
          if (resp) {
            QCRIL_LOG_INFO("acknowledgeSms_1_5 cb, result: %d", resp->errorCode);
          }
        }));
    msg->setCallback(&cb);
    msg->dispatch();
