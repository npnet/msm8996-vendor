...
void ImsRadioImpl::sendImsSmsResponse(uint32_t token, RIL_Errno errorCode,
        RIL_RadioTechnologyFamily tech, std::shared_ptr<RilSendSmsResult_t> data) {
  auto respCbV1_2 = getResponseCallbackV1_2();
  if (respCbV1_2 != nullptr) {
    ...
    V1_2::ImsSmsSendFailureReason v12Reason =
    ...
    V1_5::utils::convertRilImsResponseToHidlResponse(
                errorCode, tech, bLteOnlyReg, statusReport, v15Reason);
    v12Reason = V1_5::utils::convertHidlSendSmsFailureReasonToV12(v15Reason);
    imsRadiolog("<", "sendImsSmsResponse: token=" + std::to_string(token) +
                         " msgeRef=" + std::to_string(messageRef) + " statusReport=" +
                         toString(statusReport) + " reason=" + toString(v12Reason));
    auto ret = respCbV1_2->sendImsSmsResponse(token, messageRef, statusReport, v12Reason);
    if (!ret.isOk()) {
      QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.description().c_str());
    }
  }
...
Return<void> ImsRadioImpl::sendImsSms(int32_t token, const V1_2::ImsSmsMessage &imsSms) {
  ...
  RIL_RadioTechnologyFamily tech = (imsSms.format == "3gpp") ? RADIO_TECH_3GPP : RADIO_TECH_3GPP2;
  auto msg = std::make_shared<RilRequestImsSendSmsMessage>(getContext(token), imsSms.messageRef,
  ...
  if (msg) {
    if (tech == RADIO_TECH_3GPP) {
      ...
      if (convertPayloadToCdmaFormat(imsSms.pdu.data(), imsSms.pdu.size(), cdmaMsg)) {
  ...
  if (ret == E_SUCCESS) {
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        ([this, token, tech]([[maybe_unused]] std::shared_ptr<Message> msg,
          ...
          sendImsSmsResponse(token, resp->errorCode, tech, respData);
        }));
    msg->setCallback(&cb);
    msg->dispatch();
  } else {
    sendImsSmsResponse(token, RIL_E_GENERIC_FAILURE, tech, nullptr);
  }
...
Return<void> ImsRadioImpl::acknowledgeSms(int32_t token, uint32_t messageRef,
  ...
  auto msg = std::make_shared<RilRequestAckImsSmsMessage>(
      getContext(token), messageRef, convertHidlToRilSmsAckResult(smsdeliverResult));
  if (msg) {
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        ([token]([[maybe_unused]] std::shared_ptr<Message> msg,
                 Message::Callback::Status status,
                 std::shared_ptr<QcRilRequestMessageCallbackPayload> resp) -> void {
          (void)token;
          if (status == Message::Callback::Status::SUCCESS && resp != nullptr) {
