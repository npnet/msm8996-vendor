...
ImsModule::ImsModule() : AddPendingMessageList("ImsModule") {
  ...
  mImssIndicationHandler = qcril_qmi_imss_unsol_ind_cb_helper_v02;
  mImsGetConfigHandler = qcril_qmi_imss_request_get_ims_config_v02;
  mImsSetConfigHandler = qcril_qmi_imss_request_set_ims_config_v02;
  mImsSetServiceStatusHandler = qcril_qmi_imss_request_set_ims_srv_status_v02;
  mSetImsRegistrationChangeHandler = qcril_qmi_imss_request_set_ims_registration_v02;
...
void ImsModule::handleImssQmiIndMessage(std::shared_ptr<Message> msg) {
  ...
  QmiIndMsgDataStruct *indData = shared_indMsg->getData();
  if (indData != nullptr) {
    mImssIndicationHandler(indData->msgId, indData->indData, indData->indSize);
  } else {
    Log::getInstance().d("Unexpected, null data from message");
  }
}
...
void ImsModule::handleQcRilRequestImsRegistrationChangeMessage(
  ...
  mSetImsRegistrationChangeHandler(msg);
}
