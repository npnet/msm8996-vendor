...
static std::shared_ptr<qcril::interfaces::ImsSubConfigInfo> qcril_qmi_imsa_get_ims_sub_configs();

static std::shared_ptr<qcril::interfaces::BlockStatus>
qcril_qmi_imsa_map_imsa_block_status_to_ims_block_status
(
    imsa_ims_failure_reason_type_v01* imsa_block_status
);

static std::shared_ptr<qcril::interfaces::RegistrationBlockStatus>
copy_qmi_blk_status_to_ims_registration_block_status
(
    imsa_ims_block_status_ind_msg_v01 *src
);

static void qcril_qmi_imsa_set_geo_loc_resp_hdlr
(
   unsigned int                 msg_id,
...
static void qcril_qmi_imsa_broadcast_service_status
(
...
void qcril_qmi_imsa_init()
{
   ...
   ret = qmi_client_imsa_send_sync(QMI_IMSA_IND_REG_REQ_V01,
       (void*) &ind_reg_req,
...
void qcril_qmi_imsa_cleanup()
{
   ...
   qcril_qmi_imsa_info.ims_registration_error_string.clear();
   qcril_qmi_imsa_info.ims_registration_error_string.shrink_to_fit();
   qcril_qmi_imsa_info.registration_network_valid = FALSE;
   qcril_qmi_imsa_info.URI_List.clear();
   qcril_qmi_imsa_info.URI_List.shrink_to_fit();
   qcril_qmi_imsa_info.ims_capability.clear();
   qcril_qmi_imsa_info.ims_capability.shrink_to_fit();
      ...
      )
{
      ...
      {
        qcril_qmi_imsa_info.ims_registration_error_string = ims_registration_error_string;
      }
      qcril_qmi_imsa_info.ims_registration_error_string.shrink_to_fit();

      qcril_qmi_imsa_info.URI_List.clear();
      if (uri_list_valid && uri_list_len)
      {
        qcril_qmi_imsa_info.URI_List.reserve(uri_list_len);
        qcril_qmi_imsa_info.URI_List.insert(
            qcril_qmi_imsa_info.URI_List.begin(), URI_List, URI_List+uri_list_len);
      }
      // IMSA REFACTOR TEST
...
void qcril_qmi_imsa_reg_status_ind_hdlr(void *ind_data_ptr)
{
   ...
   if (NULL != reg_ind_msg_ptr)
   {
      ...
      {
        ims_reg_status = (reg_ind_msg_ptr->ims_registered ? IMSA_STATUS_REGISTERED_V01
  ...
  if (msg != nullptr)
  {
    do
    {
      if (qcril_qmi_imsa_info.ims_registered_valid ||
          qcril_qmi_imsa_info.ims_registration_error_code_valid ||
          !qcril_qmi_imsa_info.ims_registration_error_string.empty())
      {
        reg = qcril_qmi_ims_create_Registration(
            qcril_qmi_imsa_info.ims_registered_valid, qcril_qmi_imsa_info.ims_registered,
...
)
{
    ...
    do {
        ...
        qcril_qmi_imsa_store_ims_capability(
            qcril_qmi_imsa_info.max_ims_instances_valid,
            qcril_qmi_imsa_info.max_ims_instances,
            TRUE,
            payload->ims_capability_len,
            payload->ims_capability);
...
{
    RIL_Errno errorCode = RIL_E_SUCCESS;
    std::shared_ptr<qcril::interfaces::ImsSubConfigInfo> subConfig = nullptr;

    QCRIL_LOG_FUNC_ENTRY();

    if (msg) {
        do {
            errorCode = qcril_qmi_nas_get_device_capability(FALSE, FALSE, FALSE, TRUE);
            if (errorCode == RIL_E_SUCCESS)
            {
                subConfig = qcril_qmi_imsa_get_ims_sub_configs();
...
)
{
    ...
    qcril_qmi_imsa_info.ims_capability.clear();
    if (ims_capability_valid && ims_capability_len && ims_capability)
    {
      if (ims_capability_len <= QMI_DMS_MAX_SUBSCRIPTION_LIST_LEN_V01)
      {
        qcril_qmi_imsa_info.ims_capability.reserve(ims_capability_len);
        qcril_qmi_imsa_info.ims_capability.insert(qcril_qmi_imsa_info.ims_capability.begin(),
                                                  ims_capability,
                                                  (ims_capability + ims_capability_len));
      }
    }
    qcril_qmi_imsa_info_unlock();
    QCRIL_LOG_FUNC_RETURN();
...
//===========================================================================
std::shared_ptr<qcril::interfaces::ImsSubConfigInfo> qcril_qmi_imsa_get_ims_sub_configs()
{
  QCRIL_LOG_FUNC_ENTRY();
  std::shared_ptr<qcril::interfaces::ImsSubConfigInfo> subConfig =
  ...
  if (qcril_qmi_imsa_info.max_ims_instances_valid)
  {
    ...
    QCRIL_LOG_INFO("Max IMS instances:%d", qcril_qmi_imsa_info.max_ims_instances);
  }
  uint32_t count = 0;
  uint8_t ims_cap[RIL_SUBSCRIPTION_3+1] = {0};
  if (qcril_qmi_imsa_info.ims_capability.size() &&
      (qcril_qmi_imsa_info.ims_capability.size() <= RIL_SUBSCRIPTION_3))
  {
    for (const auto& ims_cap_entry : qcril_qmi_imsa_info.ims_capability)
    {
      if (ims_cap_entry.subscription == DMS_PRIMARY_SUBS_V01)
      {
        ims_cap[RIL_SUBSCRIPTION_1] = ims_cap_entry.enabled;
        QCRIL_LOG_INFO("Primary sub ims_cap:%d", ims_cap_entry.enabled);
        count++;
      }
      if (ims_cap_entry.subscription == DMS_SECONDARY_SUBS_V01)
      {
        ims_cap[RIL_SUBSCRIPTION_2] = ims_cap_entry.enabled;
        QCRIL_LOG_INFO("Secondary sub ims_cap:%d", ims_cap_entry.enabled);
        count++;
      }
      if (ims_cap_entry.subscription == DMS_TERTIARY_SUBS_V01)
      {
        ims_cap[RIL_SUBSCRIPTION_3] = ims_cap_entry.enabled;
        QCRIL_LOG_INFO("Tertiary sub ims_cap:%d", ims_cap_entry.enabled);
        count++;
      }
    }
  }
  for (uint8_t cap : ims_cap) {
    subConfig->addToImsStackEnabled(cap);
  }
  subConfig->setCount(count);
  QCRIL_LOG_FUNC_RETURN();
  return subConfig;
}
