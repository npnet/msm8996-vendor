...
std::shared_ptr<qcril::interfaces::Registration> qcril_qmi_ims_create_Registration(
    ...
    imsa_service_rat_enum_v01 rat, uint8_t errorCodeValid, uint16_t errorCode,
    const std::string &errorString,
    const std::vector<p_associated_uri_v01> &uriList) {
  ...
  if (reg != nullptr) {
    ...
    if (uriList.size()) {
      std::string uri_list;
      for (const auto &uriElem : uriList) {
        if (!uri_list.empty()) {
          uri_list += "|";
        }
        uri_list += uriElem.uri;
      }
...
qcril::interfaces::ServiceStatusInfo qcril_qmi_ims_create_ServiceStatusInfo(
  ...
  std::shared_ptr<qcril::interfaces::Registration> reg = qcril_qmi_ims_create_Registration(
      ...
      rat_valid, rat, FALSE, 0, std::string(), std::vector<p_associated_uri_v01>());
...
//===========================================================================
qcril_qmi_ims_config_item_type qcril_qmi_ims_map_ims_config_to_radio_config_item
(
   qcril::interfaces::ConfigItem ims_config_item
)
{
  QCRIL_LOG_FUNC_ENTRY();
  qcril_qmi_ims_config_item_type ret = QCRIL_QMI_IMS_CONFIG_INVALID;
  ...
  switch(ims_config_item)
  {
    ...
    default:
      ret = QCRIL_QMI_IMS_CONFIG_INVALID;
      break;
...
(
   qcril_qmi_ims_config_item_type radio_config_item
)
{
  QCRIL_LOG_FUNC_ENTRY();
...
(
  qcril_qmi_ims_config_settings_resp_type radio_config_error
)
{
