...
static ds_target_t ds_get_target_from_mdmdetect()
{
  ...
  /* devinfo.num_modems is 1 for SDXMARMOT */
  else if ((!(std_stricmp(devinfo.mdm_list[0].mdm_name, DS_ESOC_MDM_NAME_SDXMARMOT))) &&
           devinfo.num_modems == 1)
  {
    /* Assuming AP is SDA855 */
    ...
    do
    {
      ...
      else
      {
        ...
        switch((int)soc_id)
        {
          ...
          case DS_TARGET_SOC_ID_LAGOON_AB:
...
/*===========================================================================
  FUNCTION  ds_get_kernel_version
===========================================================================*/
/*!
@brief
   Update the Kernel version for the target.
@return
   int value signifying the success/failure of the function.
   -1: when cannot obtain system info (failure scenario)
    0: when kernel version is obtained (success scenario)
*/
/*===========================================================================*/
int ds_get_kernel_version(char *kern_ver, unsigned int ver_len)
{
  struct utsname system_info;
  int  ret = 0, rel_len = 0;

  if((kern_ver==NULL) || (ver_len<=1))
  {
    ds_log_err("Invalid request parameters");
    return -1;
  }

  memset(&system_info,0x0,sizeof(struct utsname));
  memset(kern_ver,0x0,ver_len);

  if(0==(ret=uname(&system_info)))
  {
    ds_log_med("system_info : sysname [%s] nodename [%s] release [%s] version [%s]" \
                " machine [%s]", system_info.sysname,system_info.nodename,
                system_info.release,system_info.version,system_info.machine);
  }
  else
  {
    ds_log_err("Failed to get system_info ret [%d]!!",ret);
    return -1;
  }

  rel_len = strlen(system_info.release);

  if(ver_len > rel_len)
    ver_len = rel_len+1;

  memcpy( kern_ver, &system_info.release, ver_len-1);

  ds_log_med("kern_version:[%s]", kern_ver);
  return 0;
}
