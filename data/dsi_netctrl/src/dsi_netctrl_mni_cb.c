...
)
{
  ...
  if (TRUE == client_found && st->priv.companion_ip_call )
  {
    DSI_LOG_VERBOSE("dsi_is_valid_client_found_for_iface: client found=0x%p "
                    " with companion ip call[%d]", st, st->priv.companion_ip_call);
    client_found = FALSE;
  }
  else if (TRUE == client_found)
  {
