...
/*===========================================================================
  FUNCTION:  is_qdp_profile
===========================================================================*/
/*!
    @brief
    Check whether the given profile is qdp profile or not
...
*/
/*=========================================================================*/
boolean is_qdp_profile(int index)
{
  QDP_LOG_DEBUG("is_qdp_profile for 3GPP profile id [%d]", index);
  boolean ret = FALSE;
  wds_get_profile_settings_req_msg_v01   ps_req;
  wds_get_profile_settings_resp_msg_v01  ps_resp;
  ...
  ps_req.profile.profile_type = WDS_PROFILE_TYPE_3GPP_V01;
  ps_req.profile.profile_index = index;
  ...
  {
    if(ps_resp.profile_name_valid &&
      !strncasecmp(QDP_UNIQUE_PROFILE_NAME, ps_resp.profile_name,std_strlen(QDP_UNIQUE_PROFILE_NAME)) &&
      ps_resp.persistent_valid &&
      ps_resp.persistent == FALSE)
    {
      QDP_LOG_DEBUG("Found a QDP profile @ [%d]",
                     ps_req.profile.profile_index);
      ret = TRUE;
    ...
    {
      QDP_LOG_DEBUG("%s", "It is not a qdp profile.");
    }
  }
  return ret;
}
  ...
  for(i=0; i<QDP_NUM_PROFILES_EXPECTED_MAX; i++)
  {
        ...
        profile_ids[i].qmi_type.profile_index == profile_id)
    {
      ...
      if ((0 == profile_ids[i].ref_count) &&
          (FALSE == profile_ids[i].is_persistent) &&
          (profile_ids[i].qmi_type.profile_type == WDS_PROFILE_TYPE_3GPP2_V01 ||
             is_qdp_profile(profile_id)))
      {
