...
#define NETMGR_MAIN_UL_PRIO_LOAD_PROPERTY "persist.vendor.data.qmipriod_load"
#define NETMGR_MAIN_UL_PRIO_PROPERTY_ENABLE "1"
#define NETMGR_MAIN_UL_PRIO_PROPERTY_DISABLE "0"
...
/*===========================================================================
  FUNCTION  netmgr_main_set_uplink_priority_properties
===========================================================================*/
/*!
@brief
  Sets android properties to trigger starting uplink priority handlers
...
*/
/*=========================================================================*/
int netmgr_main_set_uplink_priority_properties(boolean force_disable)
{
  char *val;

  if (force_disable || !netmgr_main_cfg.uplink_priority)
  {
    val = NETMGR_MAIN_UL_PRIO_PROPERTY_DISABLE;
  }
  else
  {
    val = NETMGR_MAIN_UL_PRIO_PROPERTY_ENABLE;
  }

  if (property_set(NETMGR_MAIN_UL_PRIO_LOAD_PROPERTY, val))
  {
    netmgr_log_err("%s(): setting property [%s] to [%s] failed", __func__,
                   NETMGR_MAIN_UL_PRIO_LOAD_PROPERTY, val);
    return NETMGR_FAILURE;
  }

  netmgr_log_low("%s(): set property [%s] to [%s]", __func__,
                 NETMGR_MAIN_UL_PRIO_LOAD_PROPERTY, val);
  return NETMGR_SUCCESS;
}

/*===========================================================================
  FUNCTION  netmgr_main_uplink_priority_init
===========================================================================*/
/*!
@brief
  Starts thread for handling uplink priority requests and ssets the properties
  for starting the dedicated daemon.
...
*/
/*=========================================================================*/
int netmgr_main_uplink_priority_init(void)
{
  int rc;
  boolean force_disable = FALSE;

  rc = netmgr_kif_uplink_priority_listener_init();
  if (rc != NETMGR_SUCCESS)
  {
    netmgr_log_high("Spawning listener failed. Disabling prioritization");
    force_disable = TRUE;
  }

  return netmgr_main_set_uplink_priority_properties(force_disable);
}
...
netmgr_main (int argc, char ** argv)
{
  ...
  netmgr_main_uplink_priority_init();
