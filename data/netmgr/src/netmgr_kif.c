...
#define NETMGR_RTR_SOLICIT_DELAY_ENABLED 1
#define NETMGR_RTR_SOLICIT_DELAY_DISABLED 0

/*===========================================================================
    FUNCTION  netmgr_kif_set_router_solicitation_delay
   ===========================================================================*/
  /*!
  @brief
    Sets the value for proc entry: router_solicitation_delay on a virtual interface.

  @arg dev_name interface to set proc entry on
  @arg state value to be set for proc entry
...
LOCAL inline int
netmgr_kif_set_router_solicitation_delay
(
  const char *dev_name,
  int random_delay
)
{
  ...
  snprintf(proc_file,
           sizeof(proc_file),
           "/proc/sys/net/ipv6/conf/%s/router_solicitation_delay",
           dev_name);

  val.intval = random_delay;
  rc = netmgr_kif_update_system_config(proc_file,
                                       NETMGR_CMDQ_TYPE_INT,
                                       val);
  if (NETMGR_SUCCESS != rc)
  {
     netmgr_log_err("%s():failed to set router_solicitation_delay on device [%s]",
                    __func__, dev_name);
  }
  return rc;
}
...
)
{
  ...
  if (netmgr_kif_set_router_solicitation_delay(link_name,
                  NETMGR_RTR_SOLICIT_DELAY_DISABLED)!=NETMGR_SUCCESS)
  {
    netmgr_log_err("%s(): Failed to set router_solicitation_delay  on transport (%s)",
                   __func__, link_name);
  }
  ...
  else if (NETMGR_IPV6_ADDR == addr_type)
  {
...
#endif
    {
      if (is_reconfig == FALSE)
      {
        ...
        cmd_rc = netmgr_kif_set_router_solicitation_delay(link_name,
                                             NETMGR_RTR_SOLICIT_DELAY_DISABLED);
        netmgr_log_high("%s(): set router_solicitation_delay on device on interface [%s] :"\
                         ":status [%s]",__func__, link_name,
                         cmd_rc == NETMGR_SUCCESS ? "SUCCESS" : "FAILURE");
...
LOCAL void netmgr_kif_iface_notify(int link, int enable)
{
  struct {
  ...
  if (!netmgr_main_cfg.uplink_priority)
	return;

  ifname = netmgr_kif_get_name(link);
  if ( (ifname == NULL) || (strncmp(ifname, "rmnet_data", 10))) {
    netmgr_log_high("%s(): Invalid link %d\n", __func__, link);
    return;
  }

  str_len = strlcpy(data.ifname, ifname, IFNAMSIZ);
  if (str_len < 0 || str_len > IFNAMSIZ) {
    netmgr_log_err("%s(): String truncation occurred\n", __func__);
    return;
  }

  sock = socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0);
  if (sock < 0) {
    netmgr_log_err("%s(): Opening unix socket failed: %s\n", __func__, strerror(errno));
    return;
  }

  memset(&sockaddr, 0, sizeof(sockaddr));
  sockaddr.sun_family = AF_UNIX;
  str_len = strlcpy(sockaddr.sun_path, NETMGR_UNIX_PRIORITY_NOTIFY_SOCK_PATH, UNIX_PATH_MAX);
  if (str_len < 0 || str_len > UNIX_PATH_MAX) {
    netmgr_log_err("%s(): Unix socket path tuncation occurred\n", __func__);
    goto close;
  }

  if (connect(sock, (struct sockaddr *)&sockaddr, sizeof(sockaddr)) < 0) {
    netmgr_log_err("%s(): Connecting to unix socket failed: %s\n", __func__, strerror(errno));
    goto close;
  }

  data.state = enable;
  bytes = send(sock, (char *)&data, sizeof(data), 0);
  if (bytes < 0) {
    netmgr_log_err("%s(): Sending notify event failed: %s\n", __func__, strerror(errno));
  } else if (bytes < sizeof(data)) {
    netmgr_log_err("%s(): Notify event was truncated\n", __func__);
  } else {
    netmgr_log_high("%s(): Notify event sent successfully for link %d\n", __func__, link);
  }
  ...
  if( ifinfo->ifi_change & IFF_UP ) {
    if( ifinfo->ifi_flags & IFF_UP ) {
      ...
      if (is_ifindex_wlan) {
        ...
        netmgr_kif_open_cnf(link);
        netmgr_kif_iface_notify(link, 1);
      }
      ...
      if (is_ifindex_wlan) {
        ...
        netmgr_kif_iface_notify(link, 0);
      }
    ...
    case NETMGR_KIF_OPENING:
...
#endif
      {
        if (AF_INET6 == ip_family)
        {
          ...
          cmd_rc = netmgr_kif_set_router_solicitation_delay(link_name,
                                              NETMGR_RTR_SOLICIT_DELAY_ENABLED);
          netmgr_log_high("%s():Set router_solicitation_delay on interface [%s] :"\
                          "status [%s]", __func__, link_name,
                          cmd_rc == NETMGR_SUCCESS ? "SUCCESS" : "FAILURE");
        }
      }
...
netmgr_kif_init_iface( int link )
{
...
#endif
  {
    ...
    cmd_rc = netmgr_kif_set_router_solicitation_delay(link_name,
                               NETMGR_RTR_SOLICIT_DELAY_ENABLED);
    netmgr_log_high("%s(): setting router_solicitation_delay on interface [%s] : status [%s]",
                    __func__, link_name, cmd_rc == NETMGR_SUCCESS ? "SUCCESS" : "FAILURE");
  }
...
/*===========================================================================
  FUNCTION  netmgr_kif_uplink_priority_transact
===========================================================================*/
/*!
@brief
  Handle receiving, executing, and returning a status code over the unix
  socket for the dedicated priority daemon.
...
*/
/*=========================================================================*/
static void netmgr_kif_uplink_priority_transact(int server_sock)
{
  char msg_buf[NETMGR_UNIX_MAX_MESSAGE_SIZE];
  int accept_sock;
  ssize_t msg_len;
  int rc;
  ...
  while (1)
  {
    accept_sock = accept4(server_sock, NULL, NULL, SOCK_CLOEXEC);
    if (accept_sock < 0) {
      netmgr_log_err("Accepting connection on unix sock failed: %s",
                     strerror(errno));
      continue;
    }

    /* Get the command */
    msg_len = recv(accept_sock, msg_buf, NETMGR_UNIX_MAX_MESSAGE_SIZE, 0);
    if (msg_len < 0)
    {
      netmgr_log_err("Receiving message on unix socket failed: %s",
                     strerror(errno));
      goto close_sock;
    }

    /* Run the command */
    rc = netmgr_ds_exec(msg_buf, NULL);
    if (rc != NETMGR_SUCCESS)
    {
      netmgr_log_err("Unix socket command failed");
    }

    /* Send back the result */
    rc = snprintf(msg_buf, NETMGR_UNIX_MAX_MESSAGE_SIZE, "%d", rc);
    msg_len = send(accept_sock, msg_buf, (size_t)rc, 0);
    if (msg_len < 0)
    {
      netmgr_log_err("Sending message on unix socket failed: %s",
                     strerror(errno));
    }
...
/*===========================================================================
  FUNCTION  netmgr_kif_ul_prio_main
===========================================================================*/
/*!
@brief
  Main function for the dedicated uplink priority handler thread
...
*/
/*=========================================================================*/
static void *netmgr_kif_ul_prio_main(void *arg)
{
  struct sockaddr_un sockaddr;
  int sock;
  int str_len;
  ...
  if (sock < 0)
  {
    netmgr_log_err("Opening unix socket failed: %s", strerror(errno));
    goto exit;
  }

  memset(&sockaddr, 0, sizeof(sockaddr));
  sockaddr.sun_family = AF_UNIX;
  str_len = strlcpy(sockaddr.sun_path, NETMGR_UNIX_PRIORITY_SOCK_PATH,
                    UNIX_PATH_MAX);
  if (str_len <= -1 || str_len > UNIX_PATH_MAX)
  {
    netmgr_log_err("Unix socket path truncated");
    goto close;
  }
  ...
  if (rc < 0)
  {
    netmgr_log_err("Binding unix socket failed: %s", strerror(errno));
    goto close;
  }

  rc = chmod(sockaddr.sun_path, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
  if (rc < 0)
  {
    netmgr_log_err("Changing unix socket permission failed: %s",
                   strerror(errno));
    goto close;
  }

  rc = listen(sock, NETMGR_UNIX_MAX_CLIENTS);
  if (rc < 0)
  {
    netmgr_log_err("Listening on unix socket failed: %s", strerror(errno));
    goto close;
  }

  netmgr_kif_uplink_priority_transact(sock);
...
/*===========================================================================
  FUNCTION  netmgr_kif_uplink_priority_listener_init
===========================================================================*/
/*!
@brief
  Function to spawn a dedicated thread for handling requests to Netmgr from
  the dedicated uplink priority daemon.

@return
  int - NETMGR_SUCCESS on successful operation, NETMGR_FAILURE otherwise
*/
/*=========================================================================*/
int netmgr_kif_uplink_priority_listener_init(void)
{
  pthread_t prio_listener;

  if (!netmgr_main_cfg.uplink_priority)
  {
    /* Well, we successfully did nothing! Huzzah! */
    return NETMGR_SUCCESS;
  }

  if (pthread_create(&prio_listener, NULL, netmgr_kif_ul_prio_main, NULL))
  {
    /* Force the prio feature to be disabled, as we cannot handle requests */
    return NETMGR_FAILURE;
  }
