...
)
{
  ...
  if (ret != CFGDB_SUCCESS)
  {
    /*Enabling hybrid qos by default, so that all filtering is done at MPSS*/
    prop_val_int = 1;
    netmgr_log_err("Error reading property [%s] Err[%d]; Setting to 0", prop_name, ret);
  }
  ...
  /* uplink_priority */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".uplink_priority");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    netmgr_log_err("Error reading property [%s] Err[%d]; Setting to 0", prop_name, ret);
  ...
  netmgr_main_cfg.uplink_priority = (boolean)prop_val_int;
...
void netmgr_config_print()
{
  ...
  netmgr_log_med("..uplink_priority        : [%u]",     netmgr_main_cfg.uplink_priority);
...
netmgr_config_check_output_mark_availability()
{
  ...
  kernel_ver = malloc(ver_len*sizeof(char));
  ret = ds_get_kernel_version(kernel_ver, ver_len);

  if(ret != NETMGR_FAILURE)
  {
    netmgr_log_high("kernel on target [%s] [%s]\n", target_str, kernel_ver);
    if(kernel_ver[0] == '5')
    {
      netmgr_log_high("enabling output mark\n");
      netmgr_main_cfg.output_mark_enable = 1;
    }
    else if(kernel_ver[0] == '4')
    {
      if(kernel_ver[2] >= '9')
      {
        netmgr_log_high("enabling output mark\n");
      ...
      else if (kernel_ver[2] >= '1' && kernel_ver[3] >= '0')
      {
        netmgr_log_high("enabling output mark\n");
        netmgr_main_cfg.output_mark_enable = 1;
      }
      ...
      {
        netmgr_log_high("disabling output mark\n");
        netmgr_main_cfg.output_mark_enable = 0;
      }
    }
    else
    {
  ...
  {
    netmgr_log_err("ds_get_kernel_version failed, defaulting to avoid output_mark\n");
    netmgr_main_cfg.output_mark_enable = 0;
  }

  netmgr_free(kernel_ver);
}
