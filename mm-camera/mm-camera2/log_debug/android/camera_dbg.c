...
/* This log debug file is for Android */
  ...
  int g_cam_log[CAM_LAST_MODULE][CAM_GLBL_DBG_INFO + 1] = {
      ...
      {0, 1, 0, 0, 0, 0, 1}, /* CAM_IFACE_MODULE  */
  ...
  void cam_debug_log(const cam_modules_t module,
    ...
    if (cam_log_fd != NULL) {
      ...
      snprintf(new_str_buffer, CDBG_MAX_STR_LEN, "Camera%s%s %d: %s: %s",
  ...
  void cam_debug_open(void) {
    ...
    if (cam_log_fd == NULL) {
      ...
      if (atoi(property_value)) {
        ...
        strlcpy(new_log_file_name, cam_log_filename, strlen(new_log_file_name));
        strlcat(new_log_file_name, pid_str, strlen(new_log_file_name));
        ...
        cam_setting_set("persist.vendor.camera.debug.logfile", "0");
