...
/**
 * Function: faceproc_get_feature_flags
 *
 * Description: Get the enable/disable flags for facial part features
 *
 * Input parameters:
 *   p_comp - The pointer to the faceproc component
 ...
 * Notes: none
 **/
void faceproc_get_feature_flags(faceproc_comp_t *p_comp,
  ...
  boolean *p_enable_gaze)
{
  ...
  *p_enable_gaze = FALSE;
  ...
  if (FD_FACEPT_ENABLE(p_comp)) {
    if ((p_comp->config.fd_face_info_mask & FACE_INFO_MASK_CONTOUR) &&
      (p_comp->fd_chromatix.enable_contour_detection)) {
      *p_enable_contour = TRUE;
    }

    if ((p_comp->config.fd_face_info_mask & FACE_INFO_MASK_SMILE) &&
      (p_comp->fd_chromatix.enable_smile_detection)) {
      *p_enable_smile = TRUE;
    }

    if ((p_comp->config.fd_face_info_mask & FACE_INFO_MASK_BLINK) &&
      (p_comp->fd_chromatix.enable_blink_detection)) {
      *p_enable_blink = TRUE;
    }

    if ((p_comp->config.fd_face_info_mask & FACE_INFO_MASK_GAZE) &&
      (p_comp->fd_chromatix.enable_gaze_detection)) {
      *p_enable_gaze = TRUE;
    }

    // We need to run PT (face landmarks) to run any of BSGC.
    if ((p_comp->config.fd_face_info_mask & FACE_INFO_MASK_FACE_LANDMARKS) ||
      (*p_enable_contour == TRUE) ||
      (*p_enable_smile == TRUE) ||
      (*p_enable_blink == TRUE) ||
      (*p_enable_gaze == TRUE)) {
  ...
  IDBG_HIGH("FD_FACEPT_ENABLE(p_comp)=%d, fd_face_info_mask=0x%x, "
    ...
    FD_FACEPT_ENABLE(p_comp), p_comp->config.fd_face_info_mask,
...
int faceproc_comp_get_param(void *handle, img_param_type param, void *p_data)
{
  ...
  case QWD_FACEPROC_RESULT: {
    ...
    if (IMG_ERROR(status)) {
      IDBG_ERROR("%s:%d] invalid faceproc result, status=%d",
  ...
  case QWD_FACEPROC_BUFF_TYPE: {
   ...
   if (atoi(value)){
     IDBG_MED("%s:%d] FD in DSP mode, alloc IMG_BUFFER_ION_ADSP",
       __func__, __LINE__);
  ...
  default:
    IDBG_ERROR("%s:%d] Error type %d", __func__, __LINE__, param);
    return IMG_ERR_INVALID_INPUT;
  }
...
#ifdef FACEPROC_USE_NEW_WRAPPER
  int rc = faceproc_sw_wrapper_load();
  if (IMG_SUCCEEDED(rc)) {
