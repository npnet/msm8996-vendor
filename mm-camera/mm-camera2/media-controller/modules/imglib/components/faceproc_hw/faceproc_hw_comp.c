...
int faceproc_hw_comp_set_param(void *handle, img_param_type parm, void *p_data)
{
  ...
  case QWD_FACEPROC_ROTATION_HINT: {
    ...
    if (p_comp->p_hw_tracker_wrapper) {
      faceproc_hw_tracker_wrapper_set_orientation(p_comp,
...
#endif
{
  ...
  p_sw_wrapper_config->filter_params.sw_face_size_perc =
  ...
  p_sw_wrapper_config->filter_params.sw_face_spread_tol =
...
static int faceproc_hw_comp_set_cfg(faceproc_hw_comp_t *p_comp, int fh)
{
  ...
  if (p_comp->p_hw_tracker_wrapper) {
    ...
    ret = faceproc_hw_tracker_wrapper_get_config(p_comp,
      &new_config);
    if (IMG_ERROR(ret)) {
  ...
  ret = faceproc_sw_wrapper_get_config(p_comp->p_sw_wrapper,
    &sw_wrapper_config);
  if (IMG_ERROR(ret)) {
    IMG_ERROR("sw_wrapper get config failed");
    goto out;
  ...
  if (p_comp->valid_chromatix) {
    ...
    faceproc_hw_comp_get_chromatix_cfg(&p_comp->fd_chromatix,
      p_face_cfg, &p_comp->tracking_info, &sw_wrapper_config);
  }

  /* If sw assisted fd is used set defaults */
  if (p_face_cfg->use_sw_assisted_fd) {
    if (!sw_wrapper_config.filter_params.sw_face_box_border_per) {
      sw_wrapper_config.filter_params.sw_face_box_border_per = 25;
    }
    if (!sw_wrapper_config.filter_params.sw_face_discard_border) {
      sw_wrapper_config.filter_params.sw_face_discard_border= 5;
    }
    if (!sw_wrapper_config.filter_params.sw_face_discard_out) {
      sw_wrapper_config.filter_params.sw_face_discard_out = 1;
    }
  }

  if (p_comp->p_sw_wrapper) {
    ret = faceproc_sw_wrapper_set_config(p_comp->p_sw_wrapper,
      &sw_wrapper_config);
    if (IMG_ERROR(ret)) {
      IMG_ERROR("sw_wrapper set config failed");
      goto out;
 ...
 *   None
 **/
void faceproc_hw_comp_frame_dump(img_frame_t *p_frame,
...
/**
 * Function: faceproc_hw_execute_sw
 *
 ...
 *   IMG_SUCCESS/IMG_ERR.
 **/
static int faceproc_hw_execute_sw(faceproc_hw_comp_t *p_comp,
  img_frame_t *p_frame, int face_cnt, faceproc_hw_face_holder_t *p_face_hold)
{
  ...
  if (profile_fd == TRUE) {
    ...
    uint32_t sw_pick_timeline =
      GET_TIME_IN_MILLIS(p_comp->fd_profile.intermediate1[profile_indx]) -
      GET_TIME_IN_MILLIS(p_comp->fd_profile.start_time[profile_indx]);
    IDBG_INFO("FDPROFILE-Timeline Frame[%d] : PickedForSWProcessing at %.3f",
  ...
  if (profile_fd == TRUE) {
    ...
    IDBG_INFO("FDPROFILE-Timeline Frame[%d] : FinishedSWProcessing at %.3f",
...
/**
 * Function: faceproc_hw_execute_sw_job
 *
 ...
 *   IMG_SUCCESS/IMG_ERR.
 **/
static int32_t faceproc_hw_execute_sw_job(void *p_appdata)
{
  faceproc_hw_exec_sw_job_t *p_exec = (faceproc_hw_exec_sw_job_t *)p_appdata;
  img_frame_t *p_frame = p_exec->p_frame;

  IDBG_HIGH("Frame[%d] : Picked Job for execution.hw_faces=%d",
    p_frame->frame_id, p_exec->face_cnt);

  faceproc_hw_execute_sw(p_exec->p_comp, p_exec->p_frame,
    p_exec->face_cnt, p_exec->face_hold);

  IDBG_HIGH("Frame[%d] : Job completed ", p_frame->frame_id);
  ...
  int result_idx, int face_cnt, faceproc_hw_face_holder_t *p_face_hold)
{
  ...
  if (p_comp->sw_thread_id) {
    ...
    if (p_exec != NULL) {
      ...
      thread_job.execute = faceproc_hw_execute_sw_job;
  ...
  if (sw_job_scheduled == false) {
    ret = faceproc_hw_execute_sw(p_comp, p_frame, face_cnt, p_face_hold);
    if (ret != IMG_SUCCESS) {
      IDBG_ERROR("Failed %d", ret);
      // No need to send Events,
      // faceproc_hw_execute_sw will send inside on both success, failure
      return ret;
    }
  }
...
void *hw_face_proc_thread_loop(void *handle)
{
  ...
  while (fail_count < FD_MAX_FAIL_CNT) {
    ...
    if (ret < 0 && errno == EINTR) {
      IDBG_ERROR("error: poll() failed error =%s %d, continue...",
...
int faceproc_hw_comp_abort(void *handle, void *p_data)
{
  ...
  if (IMG_ERROR(ret)) {
    IDBG_ERROR("Cannot move to IMG_STATE_STOP_REQUESTED state");
    return ret;;
  }
  ...
  img_type_t type)
{
  ...
  if ((count > p_comp->config.face_cfg.pending_buffs_number) &&
    ...
    IDBG_MED("Skip buffer component busy, pending_buffs_number %d , count %d ",
...
int faceproc_hw_comp_deinit(void *handle)
{
  ...
  if (p_comp->p_sw_wrapper) {
    ...
    if (IMG_ERROR(ret)) {
      IDBG_ERROR("sw_wrapper destroy failed %d", ret);
    }
...
int faceproc_hw_comp_create(void* handle, img_component_ops_t *p_ops)
{
...
#ifdef FACEPROC_USE_NEW_WRAPPER
  ret = faceproc_sw_wrapper_load();
  if (IMG_ERROR(ret)) {
    IDBG_WARN("Faceproc sw wrapper load failed. Working without them");
  }
  ...
  if (IMG_SUCCEEDED(ret)) {
    ...
    if (!p_comp->p_sw_wrapper) {
      IDBG_WARN("sw_wrapper creation failed");
    }
  }
#else
  ...
  IDBG_INFO("Using FD HW Hybrid solution : Tracker v%d + SWFP v%d",
    (p_comp->use_tracker_v2 == 0) ? 1 : 2, FD_VERSION);
