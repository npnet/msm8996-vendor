  ...
  module_cac_t *p_mod, uint32_t session_id)
{
  ...
  if (!session_params->chromatix) {
    IDBG_ERROR("%s %d: Invalid chromatix ptr", __func__, __LINE__);
    return;
  }
  IDBG_HIGH("%s %d: lux %f, gain %f, prev_lux %f, prev_gain %f, lls_flag %d",
    __func__, __LINE__,
  ...
  IDBG_MED("%s %d: LDS %d lls_flag %d", __func__, __LINE__,
    session_params->lds_enable_flag, aec_update->lls_flag);
  return;
}
