/* module_sensor.c
 *
 * Copyright (c) 2012-2017, 2019 Qualcomm Technologies, Inc. All Rights Reserved.
  ...
  cam_flash_mode_t mode, uint32_t frame_id, uint32_t identity)
{
  ...
  SLOW("SOF = %d, requested frame id = %d, apply_frame_id = %d, %d, %d",
  ...
  module_sensor_bundle_info_t *s_bundle, void *eebin_hdl)
{
  ...
  ATRACE_BEGIN("SENSOR_SD_OPEN");
  for (i = 0; i < SUB_MODULE_MAX; i++) {
    if (s_bundle->module_sensor_params[i]
         && s_bundle->module_sensor_params[i]->func_tbl.open
  ...
  mct_stream_info_t *stream_info)
{
  ...
  ATRACE_BEGIN("SENSOR_SET_RESOLUTION");
  ...
  ATRACE_BEGIN("SENSOR_SET_STREAM_CONFIG");
  ...
  if (s_bundle->last_idx < bctrl->ctrl_end_frame) {
    if (ctrl->retry_frame_skip >= 0) {
      if (!ctrl->retry_frame_skip--) {
        SHIGH("preflash on/off %d",bctrl->flash_mode);
  ...
  mct_event_t *event, module_sensor_bundle_info_t *s_bundle)
{
  ...
  ATRACE_BEGIN("SENSOR_CONFIG_PDAF");
  if (TRUE == s_bundle->is_stereo_configuration &&
    s_bundle->stereo_peer_s_bundle != NULL) {
  ...
  ATRACE_BEGIN("SENSOR_LOAD_CHROMATIX");
  ...
  ATRACE_BEGIN("SENSOR_START_STREAM");
  ...
  mct_event_control_parm_t *event_control, uint32_t identity)
{
  ...
  *stream_size_info = *(cam_stream_size_info_t *)event_control->parm_data;
  module_sensor_params = s_bundle->module_sensor_params[SUB_MODULE_SENSOR];
  RETURN_ON_NULL(module_sensor_params);
  ...
  uint32_t identity)
{
  ...
  case CAM_INTF_PARM_LED_CALIBRATION:
    s_bundle->led_calibration_mode =
      *((cam_led_calibration_mode_t *)event_control->parm_data);
    SLOW("Set led calibration mode: %d", s_bundle->led_calibration_mode);
    break;
  default:{
    sensor_output_format_t output_format;
    rc = module_sensor_params->func_tbl.process(
 ...
 *
 **/
static boolean module_sensor_fill_frame_timing(mct_module_t *module __attribute__((unused)),
  mct_event_t *event __attribute__((unused)), sensor_bundle_info_t *bundle_info)
{
  ...
  for (i = 0; i < hal_batch->num_batch; i++) {
    ...
    case CAM_CAPTURE_RESET:
      ...
      if( bctrl[i-1].type == CAM_CAPTURE_FLASH
  ...
  case MCT_EVENT_CONTROL_LINK_INTRA_SESSION: {
    ...
    if (ret == FALSE) {
      ...
      sensor_util_post_submod_event(bundle_info.s_bundle, SUB_MODULE_SENSOR,
        SENSOR_SET_DUALCAM_SETTINGS, (void*)&module_ctrl->is_dual_cam);
  ...
  void *buf, uint32_t sessionid)
{
  ...
  else if (output_format == SENSOR_GREY) {
  ...
  if (output_format == SENSOR_BAYER) {
    ...
    if (s_bundle->sensor_info->subdev_id[SUB_MODULE_EEPROM] != -1) {
      ...
      RETURN_FALSE_IF(rc < 0);
      SENSOR_SUB_MODULE_PROCESS_EVENT(s_bundle, SUB_MODULE_EEPROM,
        EEPROM_GET_ISINSENSOR_CALIB, &in_sensor, rc);
      RETURN_FALSE_IF(rc < 0);
    }

    if (wbc_factor && !in_sensor) {
