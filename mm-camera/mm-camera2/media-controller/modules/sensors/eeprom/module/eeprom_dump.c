...
void dump_dualc_data(sensor_eeprom_data_t* e_ctrl, eeprom_dump_type dump_type)
{
  ...
  if (fp_dc != NULL ) {
    ...
    fprintf(fp_dc, "Brightness ratio = %d\n",
        e_ctrl->eeprom_data.dualc.system_calib_params.
        brightness_ratio);
    fprintf(fp_dc, "Ref mono gain = %d\n",
        e_ctrl->eeprom_data.dualc.system_calib_params.
        ref_mono_gain);
    fprintf(fp_dc, "Ref mono linecount = %d\n",
        e_ctrl->eeprom_data.dualc.system_calib_params.
        ref_mono_linecount);
    fprintf(fp_dc, "Ref bayer gain = %d\n",
        e_ctrl->eeprom_data.dualc.system_calib_params.
        ref_bayer_gain);
    fprintf(fp_dc, "Ref bayer linecount = %d\n",
        e_ctrl->eeprom_data.dualc.system_calib_params.
        ref_bayer_linecount);
    fprintf(fp_dc, "Ref bayer color temperature = %d\n",
        e_ctrl->eeprom_data.dualc.system_calib_params.
        ref_bayer_color_temperature);
