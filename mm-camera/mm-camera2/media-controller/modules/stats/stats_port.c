  ...
  mct_event_t *parm_event)
{
  ...
  memcpy(post_event->event, parm_event, sizeof(mct_event_t));
  memcpy(dst_ui_parm, src_ui_parm, sizeof(mct_event_control_parm_t));
  memcpy(parm, src_ui_parm->parm_data, parm_size);
    ...
    if (stats_parm != NULL) {
      ...
      *sent_done  = TRUE;
      ...
      case CAM_INTF_PARM_WB_MANUAL: {
        ...
        memset(&manual_parm, 0, sizeof(manual_parm));
      ...
      case CAM_INTF_PARM_ISO: {
        ...
        break;
      case CAM_INTF_PARM_LED_CALIBRATION: {
        port_event.cap_flag = MCT_PORT_CAP_STATS_AWB;
        stats_parm->param_type = STATS_SET_Q3A_PARAM;
        stats_parm->u.q3a_param.type = Q3A_ALL_SET_PARAM;
        stats_parm->u.q3a_param.u.q3a_all_param.type = Q3A_ALL_SET_LED_CALIB_MODE;
        stats_parm->u.q3a_param.u.q3a_all_param.u.led_calib_mode =
          *((cam_led_calibration_mode_t *)ui_parm->parm_data);
        STATS_DBG("HALDBG:LED_CALIBRATION_MODE %d",
           stats_parm->u.q3a_param.u.q3a_all_param.u.led_calib_mode);
  ...
  enum msm_isp_stats_type stats_type, const mct_event_stats_isp_t *const stats_event)
{
  ...
  if (stats_type == MSM_ISP_STATS_BE) {
    ...
    uint32 copySize = MIN(sizeof(p_be_stats->r_sum),
    ...
    memcpy(pBeDebugStats->redChannelSum,  p_be_stats->r_sum,  copySize);
    memcpy(pBeDebugStats->grChannelSum,   p_be_stats->gr_sum, copySize);
    memcpy(pBeDebugStats->gbChannelSum,   p_be_stats->gb_sum, copySize);
    memcpy(pBeDebugStats->blueChannelSum, p_be_stats->b_sum,  copySize);
    ...
    uint32 copySize = MIN(sizeof(p_hdrbe_stats->be_r_sum),
                          sizeof(pBeDebugStats->redChannelSum));
    ...
    /* Copy BE stats sum array. */
    memcpy(pBeDebugStats->redChannelSum,  p_hdrbe_stats->be_r_sum,  copySize);
    memcpy(pBeDebugStats->grChannelSum,   p_hdrbe_stats->be_gr_sum, copySize);
    memcpy(pBeDebugStats->gbChannelSum,   p_hdrbe_stats->be_gb_sum, copySize);
    memcpy(pBeDebugStats->blueChannelSum, p_hdrbe_stats->be_b_sum,  copySize);
    ...
    memcpy(pBeDebugStats->redChannelSatSum,  p_hdrbe_stats->be_r_sat_sum,  copySize);
    memcpy(pBeDebugStats->grChannelSatSum,   p_hdrbe_stats->be_gr_sat_sum, copySize);
    memcpy(pBeDebugStats->gbChannelSatSum,   p_hdrbe_stats->be_gb_sat_sum, copySize);
    memcpy(pBeDebugStats->blueChannelSatSum, p_hdrbe_stats->be_b_sat_sum,  copySize);
  ...
  enum msm_isp_stats_type bhist_stats_type, const mct_event_stats_isp_t *const stats_event)
{
  ...
  if (debug_data_bhist_bin_size != actual_bhist_stat_bin_size)
  {
    ...
    memcpy(pHistDebugDataStats->rChannel,
           p_hist_stats->bayer_r_hist,
           sizeof(pHistDebugDataStats->rChannel[0]) * debug_data_bhist_bin_size);

    memcpy(pHistDebugDataStats->grChannel,
           p_hist_stats->bayer_gr_hist,
           sizeof(pHistDebugDataStats->grChannel[0]) * debug_data_bhist_bin_size);

    memcpy(pHistDebugDataStats->gbChannel,
           p_hist_stats->bayer_gb_hist,
           sizeof(pHistDebugDataStats->gbChannel[0]) * debug_data_bhist_bin_size);

    memcpy(pHistDebugDataStats->bChannel,
           p_hist_stats->bayer_b_hist,
...
static void send_stats_debug_data_message(mct_port_t *port)
{
  ...
  if (STATS_BUFFER_DEBUG_DATA_SIZE < total_stats_buff_size ||
    ...
    STATS_HIGH("Stats buffer debug data send error. buffer size mismatch buffer size: %d,"
...
static void send_bestats_debug_data_message(mct_port_t *port)
{
  ...
  if (BESTATS_BUFFER_DEBUG_DATA_SIZE < required_size ||
      0L == required_size ||
      0L == private->be_stats_buffer_size ||
      0L == private->be_config_buffer_size) {

     STATS_HIGH("BE Stats buffer size mismatch buffer size: %d,"
       " BE stats size: %d"
       " BE config size: %d",
       BESTATS_BUFFER_DEBUG_DATA_SIZE,
  ...
  memset(&bestats_buffer_info, 0, sizeof(cam_bestats_buffer_exif_debug_t));
  ...
  memcpy(bestats_buffer_info.bestats_buffer_private_debug_data,
         &(private->be_stats_debug_data), private->be_stats_buffer_size);

  /* Copy the BE Stats configuration */
  memcpy(bestats_buffer_info.bestats_buffer_private_debug_data + private->be_stats_buffer_size,
...
static void send_bhist_debug_data_message(mct_port_t *port)
{
  ...
  if (BHIST_STATS_DEBUG_DATA_SIZE < private->bhist_stats_buffer_size ||
       0L == private->bhist_stats_buffer_size) {

     STATS_HIGH("Bhist Stats buffer size mismatch buffer size: %d,"
       " BHist stats size: %d",
  ...
  memset(&bhist_stats_buffer_info, 0, sizeof(cam_bhist_buffer_exif_debug_t));
  ...
  if (private->bhist_stats_buffer_size > 0L) {
    ...
    memcpy(bhist_stats_buffer_info.bhist_private_debug_data,
