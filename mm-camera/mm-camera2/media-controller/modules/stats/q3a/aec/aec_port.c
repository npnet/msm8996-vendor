...
static inline void aec_port_print_bus(const char* trigger, aec_port_private_t *private)
{
  ...
  AEC_LOW("BSUPD:%15s SOF=%d,streamTy=%d,zsl_cap=%d,"
    "AEINFO:ET=%f,iso=%d,flashNeed=%d,settled=%d,bright=%f,"
    ...
    private->aec_info.flash_needed, private->aec_info.settled, private->aec_info.brightness,// AE_INFO end
  ...
  mct_pipeline_session_data_t *session_data, unsigned int *sessionid)
{
  ...
  AEC_LOW("AEC OTP data: [%d %d %d %d %d %d]",
      ...
      session_data->dual_data.ref_mono_linecount,
      session_data->dual_data.ref_mono_gain,
      session_data->dual_data.ref_bayer_linecount,
      session_data->dual_data.ref_bayer_gain,
      session_data->dual_data.ref_bayer_color_temperature);

  private->dual_cam_info.otp_data_valid = FALSE;
  if (session_data->is_valid_dualcalib &&
      session_data->dual_data.ref_mono_linecount > 0 &&
      session_data->dual_data.ref_bayer_linecount > 0) {
    uint16_t mono_linecount  = session_data->dual_data.ref_mono_linecount;
    uint16_t bayer_linecount = session_data->dual_data.ref_bayer_linecount;
    float mono_gain          = (float)session_data->dual_data.ref_mono_gain / (float)Q10;
    float bayer_gain         = (float)session_data->dual_data.ref_bayer_gain / (float)Q10;
    float brightness_ratio   = (float)session_data->dual_data.brightness_ratio / (float)Q10;
    float mono_sensitivity   = ((float)mono_linecount * mono_gain);
    float bayer_sensitivity  = ((float)bayer_linecount * bayer_gain);

    if ((bayer_sensitivity * brightness_ratio) > 0.0f) {
      private->dual_cam_info.exp_multiplier_otp =
        mono_sensitivity / (bayer_sensitivity * brightness_ratio);
      private->dual_cam_info.tuned_color_temp = session_data->dual_data.ref_bayer_color_temperature;
      AEC_LOW("AEC OTP calibration data present. Exposure multiplier: %f", private->dual_cam_info.exp_multiplier_otp);
  ...
  if (!private->dual_cam_info.otp_data_valid) {
    ...
    AEC_LOW("AEC OTP calibration data is not present");
  }
  ...
  mct_bus_msg_type_t bus_msg_type, void* payload, int size, int sof_id)
{
  ...
  if (aec_port->dual_cam_info.sensor_type != CAM_TYPE_MAIN &&
  ...
  /* Forward the bus message to peer if we are the master and the peer is linked */
  if (aec_port->dual_cam_info.sensor_type == CAM_TYPE_MAIN &&
      aec_port->dual_cam_info.peer_id != 0) {
    bool result = false;
    /* Forward the event to the slave session */
    result = stats_util_post_intramode_event(port,
                                             aec_port->dual_cam_info.peer_id,
                                             MCT_EVENT_MODULE_STATS_PEER_EVENT,
                                             (void*)&slave_event);
    if (!result) {
      AEC_MSG_ERROR("Error! failed to forward the AEC event to the slave (bus msg type=%d)",
                    bus_msg_type);
    }
  }
...
static void aec_port_send_max_flash_current_to_algo(mct_port_t *port)
{
  ...
  q3a_thread_aecawb_msg_t *aec_msg = aec_port_malloc_msg(MSG_AEC_SET,
    AEC_SET_PARAM_MAX_FLASH_CURRENT);
  ...
  uint32_t regular_sof_id)
{
  ...
  /* Secondary cam uses main cam's state, saved previously. */
  if (private->dual_cam_info.sensor_type == CAM_TYPE_MAIN) {
    aec_urgent.aec_state = private->aec_state;
  } else {
    aec_urgent.aec_state = private->aec_state_saved;
  }
  ...
  mct_port_t *port)
{
  ...
  private = (aec_port_private_t *)(port->port_private);
  /* No need to configure stats for secondary camera, unless stats are
     needed for debug data */
  if (private->dual_cam_info.sensor_type != CAM_TYPE_MAIN &&
  ...
  stats_update_t* aec_update)
{
  ...
  if (aec_port != NULL &&
      aec_port->dual_cam_info.sensor_type == CAM_TYPE_MAIN &&
      aec_port->dual_cam_info.peer_id != 0)
  {
...
/** aec_port_get_dual_cam_multiplier:
  ...
  * itself is then interpolated from its tuned value based on the current AWB color
  * temperature.
  *
  ...
  aec_port_peer_aec_update* master_update_info)
{
  ...
  if (dual_cam_info->otp_data_valid &&
    ...
    AEC_LOW("Using OTP multiplier of %f for mono interpolation", multiplier);
  } else {
    multiplier = dual_cam_info->exp_multiplier_tuning;
    AEC_LOW("Using tuning multiplier of %f for mono interpolation", multiplier);
  }
  ...
  AEC_LOW("Dual interpolation: Using adjusted multiplier value of %f at %dK temperature for mono interpolation",
    multiplier, awb_colortemp);
  ...
  stats_update_t*           slave_stats_update)
{
  ...
  if (multiplier >= 1.0f){
    ...
    slave_gain     = master_update->sensor_gain * res_multiplier;
  }
  else {
  ...
  if (master_update->total_drc_gain > 0.0f) {
    slave_update->real_gain = slave_gain * master_update->total_drc_gain;
  } else {
  ...
  slave_update->lux_idx   = master_update->lux_idx;

  AEC_LOW("Dual interpolation: Master: Exp_ time %f SG:RG:LC=(%02.3f:%02.3f:%04d)"
    " exp_index: %d, lux_idx: %f ISO=%d DRC_Gains: (TG:CG=%02.3f:%02.3f), DRC_Ratios(%1.2f,%1.2f,%1.2f,%1.2f)",
       ...
       master_update->exif_iso,
       master_update->total_drc_gain, master_update->color_drc_gain,
       master_update->gtm_ratio, master_update->ltm_ratio, master_update->la_ratio, master_update->gamma_ratio);

  AEC_LOW("Dual interpolation: Slave : Exp_ time %f SG:RG:LC=(%02.3f:%02.3f:%04d)"
    " exp_index: %d, lux_idx: %f ISO=%d DRC_Gains: (TG:CG=%02.3f:%02.3f), DRC_Ratios(%1.2f,%1.2f,%1.2f,%1.2f)",
       ...
       slave_update->exif_iso,
       slave_update->total_drc_gain, slave_update->color_drc_gain,
       slave_update->gtm_ratio, slave_update->ltm_ratio, slave_update->la_ratio, slave_update->gamma_ratio);
  ...
  aec_port_peer_aec_update* aec_update)
{
  ...
  if(aec_port->dual_cam_info.is_low_power_mode_enabled) {
    AEC_LOW("Dual interpolation: Retun with low power mode");
    return;
  }
...
/** aec_port_handle_peer_event:
  ...
  *
  * Handles forwarding of an MCT event from master session
  *
  * Return: void
  */
  ...
  mct_event_t* event)
{
  ...
  if (MCT_EVENT_MODULE_STATS_POST_TO_BUS == event->u.module_event.type) {
    ...
    if (busmsg->type == MCT_BUS_MSG_AEC_IMMEDIATE) {
      ...
      aec_port->aec_state_saved = (uint8_t)aec_urgent->aec_state;
      return;
    }
  }
  if(aec_port->dual_cam_info.is_low_power_mode_enabled) {
  ...
  aec_port_private_t *private)
{
  ...
  stats_update->aec_update.led_state = Q3A_LED_OFF;
...
static boolean aec_port_link_to_peer(mct_port_t *port,
  ...
  if (rc == FALSE) {
    ...
    private->dual_cam_info.peer_id = peer_identity;
    private->dual_cam_info.sensor_type = link_param->type;
 ...
 *    @port: private AEC port data
 *
 * Resets peer information
 *
 * Returns void
 **/
void aec_port_reset_peer(mct_port_t *port)
{
  aec_port_private_t *private = (aec_port_private_t *)(port->port_private);
  private->dual_cam_info.peer_id = 0;
  private->dual_cam_info.sensor_type = CAM_TYPE_MAIN;
}

/** aec_port_unlink_from_peer:
 *    @port: private AEC port data
 *
 * Unlinks from AEC peer, if linked
 *
 * Returns void
 **/
static void aec_port_unlink_from_peer(mct_port_t *port, mct_event_t* event)
{
  aec_port_private_t *private = (aec_port_private_t *)(port->port_private);
  mct_port_t *peer_port = NULL;
  boolean rc = FALSE;

  /* No peer to unlink from */
  if (private->dual_cam_info.peer_id == 0){
    return;
  }

  rc = stats_util_get_peer_port(event, private->dual_cam_info.peer_id, port, &peer_port);
  if (rc == FALSE) {
    AEC_ERR("Failed to get the AEC peer port");
  } else {
    MCT_PORT_INTRAUNLINKFUNC(peer_port);
    aec_port_reset_peer(peer_port);
  }

  MCT_PORT_INTRAUNLINKFUNC(port);
  aec_port_reset_peer(port);
}
  ...
  mct_event_t *event)
{
    ...
    if (stat_parm->param_type == STATS_SET_Q3A_PARAM) {
      ...
      if (q3a_param->type == Q3A_SET_AEC_PARAM) {
        ...
        case Q3A_ALL_SET_DO_LED_EST_FOR_AF: {
          ...
          const boolean use_sync = (TRUE == previous_state && FALSE == new_state) ?
            TRUE : FALSE;
        ...
        case Q3A_ALL_SET_LED_CALIB_MODE: {
          q3a_thread_aecawb_msg_t *aec_msg = aec_port_malloc_msg(MSG_AEC_SET,
            AEC_SET_PARAM_LED_CALIB_ENABLE);
          if (aec_msg != NULL ) {
            switch (q3a_param->u.q3a_all_param.u.led_calib_mode) {
            case CAM_LED_CALIBRATION_MODE_OFF:
              aec_msg->u.aec_set_parm.u.led_calib_enable = 0;
              break;
            case CAM_LED_CALIBRATION_MODE_DUAL:
            case CAM_LED_CALIBRATION_MODE_SINGLE:
              aec_msg->u.aec_set_parm.u.led_calib_enable = 1;
              break;
            default:
              AEC_ERR("Invalid led calibration mode");
              rc = FALSE;
              break;
            }
            if (rc)
        ...
        case Q3A_ALL_SET_CONFIG_AE_SCAN_TEST: {
          q3a_thread_aecawb_msg_t *aec_msg = aec_port_malloc_msg(MSG_AEC_SET,
            AEC_SET_PARAM_CONFIG_AE_SCAN_TEST);
          if (aec_msg != NULL ) {
 ...
 case MCT_EVENT_CONTROL_SET_SUPER_PARM: {
  ...
  case MCT_EVENT_CONTROL_HW_WAKEUP:{
    AEC_HIGH("CAM-TYPE=%d AEC Got HW_WAKEUP Command",private->dual_cam_info.sensor_type);
    private->dual_cam_info.is_low_power_mode_enabled = FALSE;
    }
    break;

  case MCT_EVENT_CONTROL_HW_SLEEP: {
    AEC_HIGH("CAM-TYPE=%d AEC got HW_SLEEP Command",private->dual_cam_info.sensor_type);
    private->dual_cam_info.is_low_power_mode_enabled = TRUE;
    }
    break;
      ...
      if (TRUE == rc) {
        ...
        if (private->stream_type == CAM_STREAM_TYPE_SNAPSHOT) {
          ...
          if (private->manual.is_exp_time_valid || private->manual.is_gain_valid) {
            ...
            stats_get_data->aec_get.real_gain[0] = out_real_gain;
            stats_get_data->aec_get.sensor_gain[0] = out_sensor_gain;
            stats_get_data->aec_get.linecount[0] = out_linecount;
            stats_get_data->aec_get.exp_time     = out_exp_time;
          }
        ...
        AEC_HIGH("AEUPD: GETDATA-AEC: G(%f, %f), LC=%u, DRC Gain(%f, %f),"
          " DRC Ratios(%f, %f, %f, %f), lux=%f, ET=%f",
          out->real_gain[0], out->sensor_gain[0], out->linecount[0], out->total_drc_gain,
          out->color_drc_gain, out->gtm_ratio, out->ltm_ratio, out->la_ratio,
          out->gamma_ratio,out->lux_idx, out->exp_time);

        /* Update exif information */
        memset(&private->aec_info, 0, sizeof(cam_3a_params_t));
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_SET_CHROMATIX_PTR: {
    ...
    if (aec_msg != NULL ) {
      ...
      memset(&private->chromatix_wrapper, 0, sizeof(chromatix_3a_parms_wrapper));
      parse_chromatix(mod_chrom->chromatix3APtr, &private->chromatix_wrapper);
      aec_msg->u.aec_set_parm.u.init_param.chromatix = (void*)&private->chromatix_wrapper;
      aec_msg->u.aec_set_parm.u.init_param.warm_start.stored_params = private->stored_params;

      rc = q3a_aecawb_thread_en_q_msg(private->thread_data, aec_msg);
      private->init_sensitivity =
         (private->chromatix_wrapper.AEC.aec_exposure_table.exposure_entries[0].gain /256.0) *
         private->chromatix_wrapper.AEC.aec_exposure_table.exposure_entries[0].line_count;
      private->ISO100_gain =
        private->chromatix_wrapper.AEC.aec_generic.ISO100_gain;
      private->fast_aec_forced_cnt = private->chromatix_wrapper.AEC.aec_generic.fast_aec_forced_cnt;
      ...
      float inverse_multiplier = private->chromatix_wrapper.AEC.aec_generic.dual_cam_exposure_mult;
      if (inverse_multiplier != 0.0f) {
        private->dual_cam_info.exp_multiplier_tuning =  1.0f / inverse_multiplier;
      } else {
      ...
      if (atof(value) != 0.0f){
        private->dual_cam_info.exp_multiplier_tuning = atof(value);
      }
      if (private->dual_cam_info.exp_multiplier_tuning <= 0.0f){
        private->dual_cam_info.exp_multiplier_tuning = 1.0f;
      }
      ...
      private->dual_cam_info.tuning_table[0].compensation_ratio = private->chromatix_wrapper.AEC.aec_generic.dual_cam_comp_ratio[0];
      private->dual_cam_info.tuning_table[1].color_temperature  = 5000;    //D50
      private->dual_cam_info.tuning_table[1].compensation_ratio = private->chromatix_wrapper.AEC.aec_generic.dual_cam_comp_ratio[1];
      private->dual_cam_info.tuning_table[2].color_temperature  = 4500;    //CW
      private->dual_cam_info.tuning_table[2].compensation_ratio = private->chromatix_wrapper.AEC.aec_generic.dual_cam_comp_ratio[2];
      private->dual_cam_info.tuning_table[3].color_temperature  = 4200;    //TL84
      private->dual_cam_info.tuning_table[3].compensation_ratio = private->chromatix_wrapper.AEC.aec_generic.dual_cam_comp_ratio[3];
      private->dual_cam_info.tuning_table[4].color_temperature  = 3200;    //A
      private->dual_cam_info.tuning_table[4].compensation_ratio = private->chromatix_wrapper.AEC.aec_generic.dual_cam_comp_ratio[4];
      private->dual_cam_info.tuning_table[5].color_temperature  = 2600;    //Horizon
      private->dual_cam_info.tuning_table[5].compensation_ratio = private->chromatix_wrapper.AEC.aec_generic.dual_cam_comp_ratio[5];
    ...
    if (aec_msg != NULL ) {
      ...
      rc = q3a_aecawb_thread_en_q_msg(private->thread_data, aec_msg);
        ...
        if (private->is_first_crop_info == TRUE && private->aec_auto_mode != AEC_MANUAL) {
          AEC_LOW("First CROP INFO - Trigger CB");
          /* Trigger to store initial parameters */
          q3a_thread_aecawb_msg_t *aec_msg1= aec_port_malloc_msg(MSG_AEC_SEND_EVENT,
  ...
  case MCT_EVENT_MODULE_STATS_LIGHT_UPDATE: {
    ...
    q3a_thread_aecawb_msg_t *aec_msg = aec_port_malloc_msg(MSG_AEC_SET,
      AEC_SET_PARAM_LIGHT_SENSOR_INFO);
    if (!aec_msg) {
...
static boolean aec_port_intra_event(mct_port_t *port, mct_event_t *event)
{
  ...
  switch(event->u.module_event.type)
  {
    ...
    case MCT_EVENT_MODULE_STATS_PEER_EVENT:
      AEC_LOW("Received MCT_EVENT_MODULE_STATS_PEER_EVENT");
      aec_port_handle_peer_event(
          port,
