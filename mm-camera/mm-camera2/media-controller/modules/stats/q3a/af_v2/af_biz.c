...
void af_biz_util_update_output_data(af_biz_internal *af,
  ...
  case AF_OUTPUT_RESET_LENS:
  {
    ...
    memcpy(&af->af_out.reset_lens, (af_reset_lens_t *)data,
...
inline void print_step_table(const char *str, af_step_size_adapter_t *light) {
...
inline void print_bv_threshold(const char *str, BV_threshold_adapter_t *BV_thres) {
#if !defined(LOG_DEBUG)
  CAM_UNUSED_PARAM(str);
...
static int af_biz_focus_mode_internal(af_biz_internal *af,
  ...
  if (af->af_input.tuning_info == NULL) {
    AF_ERR("Return tuning pointer NULL");
    return 0;
  }
...
void af_biz_handle_stats_config_paaf(af_biz_internal *af,
  ...
  sw_stats_config->roi.dy = (uint16_t)core_stats_info->roi.width;
  ...
  bf_fw_roi_cfg_t *af_fw_roi_cfg)
{
  ...
  for (row = 0; row < (int32_t)config.total_row &&
    ...
    for (col = 0; col < (int32_t)config.total_col &&
      ...
      AF_LOW("ROI[%d][%d, %d]: x=%d, y=%d, dx=%d, dy=%d",
...
void *af_biz_init(void *libptr)
{
  ...
  do {
    ...
    result = af->af_algo_ops.init(&af->af_core);
...
boolean af_biz_set_parameters(af_set_parameter_t *param,
  af_output_data_t *output, void *af_internal) {
  boolean rc            = TRUE;
  ...
  AF_LOW(" param=%p af_obj=%p, type =%d", param, af, param->type);
  /* Reset af_out data structure */
  memset(&af->af_out, 0, sizeof(af_output_data_t));
  memset(output, 0, sizeof(af_output_data_t));
  ...
  case AF_SET_PARAM_INIT_CHROMATIX_PTR:
    af->af_input.tuning_info = param->u.af_init_param.tuning_info;
...
// Dont need it any more.
  ...
  case AF_SET_PARAM_SET_PEER_FOCUS_INFO: {
    ...
    break;
  case AF_SET_PARAM_SET_FOLLOW_MODE: {
      set_param.param_type = AF_CORE_SET_FOLLOW_MODE;
      set_param.u.af_follow_mode = param->u.af_follow_mode;
      af->af_algo_ops.set_parameters(af->af_core, &set_param, &af->af_core_out);
      need_callback = TRUE;
    }
    break;
  ...
  if(!af || !need_callback) return FALSE;
