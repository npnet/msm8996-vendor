    ...
    uint32_t buf_alignment, uint32_t min_stride, uint32_t min_scanline)
{
  ...
  if (rc==0) {
    ...
    CDBG_ERROR("%s: returned fail=%d",__func__,rc);
  }
  ...
  boolean             is_burst)
{
 ...
 iface_util_decide_zsl_mode(session, reserving_pix_stream,
  ...
  if (((session->cds_feature_enable) && ((session->cds_saved_state &&
    ...
    (mct_stream->stream_info.stream_type != CAM_STREAM_TYPE_OFFLINE_PROC)) {
      ...
      if (isp_sink_port->event_func(isp_sink_port, &event)) {
        ...
        for (i = 0; i < IFACE_MAX_STREAMS; i++) {
          if ((session->hw_streams[i].stream_info.dim.width != 0) &&
            (session->hw_streams[i].stream_info.cam_stream_type !=
  ...
  reserving_pix_stream->sensor_skip_cnt = initial_frame_skip;
  ...
  uint32_t isp_id)
{
 ...
 /*  We need to ensure that this clock rate is good enough.
  ...
  if ((num_rdi > 0) && (!split_info->is_split)) {
  ...
  iface_resource_request_t *isp_resource)
{
  ...
  iface_session->session_resource.camif_cnt += isp_resource->num_pix_stream;
  ...
  iface_port_t *iface_sink_port, uint32_t session_id, uint32_t stream_id)
{
  ...
  if (iface_session->hw_version == ISP_MSM8937 ||
      ...
      iface_session->hw_version == ISP_MSM8956 ||
...
/** iface_util_reset_used_resource_mask
 *
 * @isp_id: isp hw id
 * @iface_session: session information
 *
 *  This function releases the used resource mask when hw stream is
 *  turned off. This helps to reuse same ispif intf can be used
 *  for next streamon.
 *
 **/
int32_t iface_util_reset_used_resource_mask(iface_t *iface, int isp_id,
  iface_session_t *iface_session)
{
  int32_t  rc = 0;
  ...
  if ((iface_session->session_resource.used_resource_mask &
  ...
  if ((iface_session->session_resource.used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI0)))) {
      iface_session->session_resource.used_resource_mask &=
        ~(1 << (16 * isp_id + IFACE_INTF_RDI0));
      CDBG("%s: RDI0 isp_id %x\n", __func__, isp_id);
  }
  if ((iface_session->session_resource.used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI1)))) {
      iface_session->session_resource.used_resource_mask &=
        ~(1 << (16 * isp_id + IFACE_INTF_RDI1));
      CDBG("%s: RDI1 isp_id %x\n", __func__, isp_id);
  }
  if ((iface_session->session_resource.used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI2)))) {
      iface_session->session_resource.used_resource_mask &=
        ~(1 << (16 * isp_id + IFACE_INTF_RDI2));
      CDBG("%s: RDI2 isp_id %x\n", __func__, isp_id );
  }
  if ((iface->used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_PIX)))) {
      iface->used_resource_mask &=
        ~(1 << (16 * isp_id + IFACE_INTF_PIX));
      CDBG("%s: PIX isp_id %x\n", __func__, isp_id);
  }
  if ((iface->used_resource_mask &
 ...
 *
 **/
static int iface_get_rdi_resource(int isp_id, iface_session_t *iface_session,
  iface_intf_type_t *reserved_rdi_intf, iface_t *iface)
{
  uint32_t isp_id_mask = 1 << isp_id;
  ...
  CDBG("%s: isp_id= %d used_resource_mask= %x\n", __func__,
    isp_id, iface_session->session_resource.used_resource_mask);
  if ((iface_session->session_resource.used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI0))) == 0 &&
    (iface->used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI0))) == 0) {
      *reserved_rdi_intf = IFACE_INTF_RDI0;
      iface_session->session_resource.isp_id_mask |= isp_id_mask;
      iface_session->session_resource.used_resource_mask |=
        1 << (16 * isp_id + IFACE_INTF_RDI0);
      iface->used_resource_mask |=
        1 << (16 * isp_id + IFACE_INTF_RDI0);
      CDBG("%s: RDI0 isp_id %d\n", __func__, isp_id);
  } else if ((iface_session->session_resource.used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI1))) == 0 &&
    (iface->used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI1))) == 0) {
      *reserved_rdi_intf = IFACE_INTF_RDI1;
      iface_session->session_resource.isp_id_mask |= isp_id_mask;
      iface_session->session_resource.used_resource_mask |=
        1 << (16 * isp_id + IFACE_INTF_RDI1);
      iface->used_resource_mask |=
        1 << (16 * isp_id + IFACE_INTF_RDI1);
      CDBG("%s: RDI1 isp_id %d\n", __func__, isp_id);
  } else if ((iface_session->session_resource.used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI2))) == 0 &&
    (iface->used_resource_mask &
    (1 << (16 * isp_id + IFACE_INTF_RDI2))) == 0) {
      *reserved_rdi_intf = IFACE_INTF_RDI2;
      iface_session->session_resource.isp_id_mask |= isp_id_mask;
      iface_session->session_resource.used_resource_mask |=
        1 << (16 * isp_id + IFACE_INTF_RDI2);
      iface->used_resource_mask |=
        1 << (16 * isp_id + IFACE_INTF_RDI2);
      CDBG("%s: RDI2 isp_id %d\n", __func__, isp_id);
  } else
     return -1;
  ...
  uint32_t          *isp_id)
{
  ...
  if ((session->session_resource.isp_id_mask & (1 << VFE0)) &&
    session->session_resource.used_wm[VFE0] < NUM_AXI_WM &&
    iface->used_wm[VFE0] < NUM_AXI_WM ) {
    /* No isp or isp0 is used by the session. get RDI from isp 0 */
    *isp_id = VFE0;
    rc = iface_get_rdi_resource(VFE0, session, reserved_rdi_intf,iface);
  } else if ((session->session_resource.isp_id_mask & (1 << VFE1)) &&
    session->session_resource.used_wm[VFE1] < NUM_AXI_WM &&
    iface->used_wm[VFE1] < NUM_AXI_WM) {
    *isp_id = VFE1;
    rc = iface_get_rdi_resource(VFE1, session, reserved_rdi_intf, iface);
  } else {
    if ((session->session_resource.used_resource_mask &
        (1 << (16 * VFE0 + IFACE_INTF_RDI0))) == 0 &&
        (session->session_resource.used_resource_mask &
        (1 << (16 * VFE0 + IFACE_INTF_RDI1))) == 0 &&
        (session->session_resource.used_resource_mask &
        (1 << (16 * VFE0 + IFACE_INTF_RDI2))) == 0) {
      *isp_id = VFE0;
      rc = iface_get_rdi_resource(VFE0, session, reserved_rdi_intf, iface);
    } else if ((session->session_resource.used_resource_mask &
        (1 << (16 * VFE1 + IFACE_INTF_RDI0))) == 0 &&
        (session->session_resource.used_resource_mask &
        (1 << (16 * VFE1 + IFACE_INTF_RDI1))) == 0 &&
        (session->session_resource.used_resource_mask &
        (1 << (16 * VFE1 + IFACE_INTF_RDI2))) == 0) {
      *isp_id = VFE1;
      rc = iface_get_rdi_resource(VFE1, session, reserved_rdi_intf, iface);
    } else {
      /* should not go here */
  ...
  if (rc < 0) {
    CDBG_HIGH("%s:VFE%d out of RDI, reserve another VFE\n",
      __func__, rc, *isp_id);
    *isp_id = (*isp_id == VFE0)? VFE1: VFE0;
    rc = iface_get_rdi_resource(*isp_id, session, reserved_rdi_intf, iface);
  }
 ...
 **/
enum msm_vfe_dual_hw_ms_type iface_util_map_cam_sync_type_to_iface(
  ...
  mct_bus_msg_t *mct_bus_data)
{
  ...
  if (post_fe_done) {
      rd_done_event = (mct_bus_msg_isp_rd_done_t *)mct_bus_data->msg;
      rd_done_event->frame_id = session->offline_info.offline_frame_id;
      rc = mct_module_post_bus_msg(iface->module, mct_bus_data);
  }
  return rc;
}
...

{
  ...
  CDBG_HIGH("%s:%d DIS enable flag = %d\n", __func__, __LINE__,
        session->dis_enabled);
  return 0;
}
                ...
                cam_stream_size_info_t *streams_desc)
{
  ...
  session->ms_type = iface_util_map_cam_sync_type_to_iface(
    streams_desc->sync_type);
  CDBG_HIGH("%s: ms_type %d \n", __func__, session->ms_type);
  ...
  iface_t *iface, iface_param_frame_request_t *frame_request)
{
  ...
  kernel_sofid = *(uint32_t *)axi_hw->mmap_addr;
  ...
  iface_session_t *session, int32_t *skip_pattern)
{
  ...
  if (session->dynamic_stats_skip_feature_enb == FALSE) {
    CDBG_ERROR("%s: dynamica stats skip feature not enabled!\n", __func__);
    return 0;
  }
  ...
  iface_frame_divert_notify_t *divert_event)
{
  ...
  if (!hw_stream->divert_to_3a) {
    ...
    if (session->overflow_recovery_state !=
      ...
      CDBG_ERROR("%s: HW error! frame id reverse!! sessid %d, hw streamid %X\n",
        __func__, user_stream->session_id,
        hw_stream->hw_stream_id);
      CDBG_ERROR("%s: curr: frame %d, sec %ld usec %ld\n",
        __func__, buf_divert->frame_id, buf_divert->timestamp.tv_sec,
        buf_divert->timestamp.tv_usec);
      CDBG_ERROR("%s: prev: frame %d, sec %ld usec %ld\n",
  ...
  uint32_t                  hw_stream_id)
{
  ...
  if (fetch_cfg_cmd == ISP_FETCH_PIPELINE_START) {
      ...
      hw_stream->need_uv_subsample =
        fetch_engine_cfg_request.cds_enable;
  }
  ...
  int                  axi_idx)
{
  ...
  if (session->overflow_recovery_state ==
    ...
    if (sof_event->frame_id <= session->sof_frame_id) {
      ...
      session->recovery_done_sof_frame_id = sof_event->frame_id;
      IFACE_HIGH("Recovery done at SOF %d\n", sof_event->frame_id);
      session->overflow_recovery_state = IFACE_AXI_RECOVERY_STATE_RECOVERY_DONE;
    }
  }

  if ((session->overflow_recovery_state == IFACE_AXI_RECOVERY_STATE_RECOVERY_DONE) &&
      (sof_event->frame_id < session->recovery_done_sof_frame_id)) {
      IFACE_HIGH("Skip posted SOF %d during recovery \n", sof_event->frame_id);
      pthread_mutex_unlock(&session->mutex);
      return 0;
  }
  ...
  iface_port_t *iface_sink_port, uint32_t isp_id, uint32_t *vfe_clk)
{
  ...
  for (i = 0; i < IFACE_MAX_STREAMS; i++) {
    ...
    if (hw_stream->stream_type == IFACE_STREAM_TYPE_META_DATA) {
      ...
      input_cfg.d.rdi_cfg.cid =
        sink_port->sensor_cap.sensor_cid_ch[primary_cid_idx].cid;
    }
 ...
 *
 **/
int iface_util_reset_resource_mask(iface_t *iface, iface_session_t *iface_session,
  uint32_t interface_mask, uint32_t isp_id_mask)
{
  ...
  for (i = 0; i < VFE_MAX; i++) {
    if (isp_id_mask & (1 << i) && (interface_mask & (0xffff << (16 * i)))) {
      /* release isp0 interface mask*/
      iface_util_reset_used_resource_mask(iface, i, iface_session);
      if ((interface_mask & (1 << (16 * i + IFACE_INTF_PIX)))) {
        interface_mask &= ~(1 << (16 * i + IFACE_INTF_PIX));
      } else if (interface_mask & (1 << (16 * i + IFACE_INTF_RDI0))) {
        interface_mask &= ~(1 << (16 * i + IFACE_INTF_RDI0));
      } else if (interface_mask & (1 << (16 * i + IFACE_INTF_RDI1))) {
        interface_mask &= ~(1 << (16 * i + IFACE_INTF_RDI1));
      } else if (interface_mask & (1 << (16 * i + IFACE_INTF_RDI2))) {
        interface_mask &= ~(1 << (16 * i + IFACE_INTF_RDI2));
      } else
        rc = -1;
    }
  }
  ...
  CDBG("%s: isp_mask = 0x%x, isp_interface_mask = 0x%x",
    __func__,  isp_id_mask, interface_mask);
  ...
  iface_session_t *session, iface_hw_stream_t *hw_stream)
{
  ...
  /* unreserve current session WM */
  if (iface->used_wm[VFE0])
    iface->used_wm[VFE0]-= session->session_resource.used_wm[VFE0];
  if (iface->used_wm[VFE1])
    iface->used_wm[VFE1]-= session->session_resource.used_wm[VFE1];
  /* Reset session_resource mask if both offline & online VFE are done */
                ...
                boolean  need_adsp_heap, uint32_t num_additional_buffers)
{
  ...
  ret = iface_alloc_native_buf_handles(&iface->buf_mgr, buf_info,
           ...
           session->image_bufs[axi_stream_src]);
                ...
                iface_session_t *session, iface_hw_stream_t *hw_stream)
{
  ...
  if (session->image_bufs[hw_stream->axi_path][0].vaddr!=NULL &&
    hw_stream->stream_info.cam_stream_type != CAM_STREAM_TYPE_OFFLINE_PROC) {
