  ...
  uint32_t stream_id, iface_session_t *iface_session)
{
  ...
  ATRACE_BEGIN("IFACE:create axi hw");
  /*Create AXI HW:
  ...
  ATRACE_BEGIN("IFACE:config axi hw");
  /* config AXI HW*/
  ...
  mct_event_t     *event)
{
  ...
  if (iface_session->ms_type == MS_TYPE_NONE) {
    CDBG_ERROR("%s: Error! Invalid Params session %d ms_type %d\n",
      __func__, iface_session->session_id, iface_session->ms_type);
    return -1;
  }
  ...
  iface_port_t    *iface_sink_port __unused)
{
  ...
  iface_util_axi_streamoff(iface, session, num_hw_streams, hw_stream_ids);
  ...
  uint32_t  halt_frame_id)
{
  ...
  int                         axi_idx;
  iface_hvx_t                 *hvx;
  iface_session_t             *temp_session = NULL;
  ...
  for (i = 0; i < IFACE_MAX_SESSIONS; i++) {
    temp_session = &iface->sessions[i];
    if (!(temp_session->session_resource.isp_id_mask &
      session->session_resource.isp_id_mask))
      continue;

    memset(&hw_stream_ids, 0, sizeof(uint32_t) * IFACE_MAX_STREAMS);
    num_hw_streams = iface_util_get_hw_streams_ids_in_session(iface, temp_session, hw_stream_ids);
    IFACE_HIGH("Doing ISPIF off session id 0x%X isp mask 0x%X\n", temp_session->session_id,
      temp_session->session_resource.isp_id_mask);
    rc = iface_ispif_get_cfg_params_from_hw_streams(iface, iface_sink_port,
      temp_session, num_hw_streams, hw_stream_ids, cfg_cmd, FALSE);
  ...
  for (i = 0; i < IFACE_MAX_SESSIONS; i++) {
    ...
    memset(&hw_stream_ids, 0, sizeof(uint32_t) * IFACE_MAX_STREAMS);
    num_hw_streams = iface_util_get_hw_streams_ids_in_session(iface, temp_session, hw_stream_ids);
    IFACE_HIGH("Doing ISPIF ON session id 0x%X isp mask 0x%X\n", temp_session->session_id,
      temp_session->session_resource.isp_id_mask);
    rc = iface_ispif_get_cfg_params_from_hw_streams(iface, iface_sink_port,
      temp_session, num_hw_streams, hw_stream_ids, cfg_cmd, TRUE);
    if (cfg_cmd->params.num == 0) {
      IFACE_ERR("ISPIF configure cmd is seriously wrong\n");
      rc = -1;
      goto error;
...
#ifdef VIDIOC_MSM_ISP_CFG_HW_STATE
int iface_handle_control_hw_state(
  iface_t               *iface,
  mct_module_hw_state_t  hw_state)
{
  iface_session_t *iface_session = NULL;
  int rc = 0, i, fd;

  fd = iface->isp_axi_data.axi_data[0].fd;
  rc = ioctl(fd, VIDIOC_MSM_ISP_CFG_HW_STATE, &hw_state);
  if (rc < 0 ) {
    CDBG_ERROR("%s: VIDIOC_MSM_ISP_CFG_HW_STATE is failed !!\n", __func__);
  }
  return rc;
}
#endif
...
void iface_destroy_hw(iface_t *iface, int hw_idx)
{
  ...
  if (!iface || (hw_idx >= VFE_MAX)) {
    CDBG_ERROR("%s:%d failed iface %p hw_idx %d\n", __func__, __LINE__, iface,
      hw_idx);
    return;
  }
  ...
  CDBG_HIGH("%s: Destroy axi hw on VFE :%d\n", __func__, hw_idx);
                     ...
                     int hw_idx)
{
  ...
  if (iface->isp_axi_data.axi_data[hw_idx].ref_cnt == 0) {
    CDBG_HIGH("%s:%d: Create axi hw for session %d on VFE %d\n", __func__, __LINE__,
  ...
  if (rc < 0) {
    CDBG_ERROR("%s: error in axi hw init, rc = %d\n", __func__, rc);
    rc = -1;
