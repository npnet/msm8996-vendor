...
int32_t cpp_hardware_notify_buf_done(cpp_hardware_t *cpphw, uint32_t identity)
{
  ...
  if (stream_status) {
    ...
    CPP_ERR("stream_status is NULL, iden:0x%x", identity);
  }
...
{
  int32_t rc;
  ATRACE_BEGIN("cpp_create_hw_frame");
  /* note: make sure to unlock this on each return path */
  ...
  cpp_hardware_params_t *hw_params)
{
  ...
  /* Usage: To enable dumps
    Preview: adb shell setprop vendor.camera.cpp.dumppreviewpayload x
      will dump payload of x frames.
    offline: adb shell vendor.camera.cpp.dumpreprocesspayload 1
