...
#define CPP_NUM_SINK_PORTS    15
#define CPP_NUM_SOURCE_PORTS  15
...
static void cpp_module_loglevel()
{
  ...
  property_get("persist.vendor.camera.cpp.mod.mask", cpp_prop, "255");
      ...
      if(ctrl->session_params[i]->session_id == session_id) {
        for(j=0; j < CPP_MODULE_MAX_STREAMS; j++) {
          if(ctrl->session_params[i]->stream_params[j] == NULL) {
            ...
            if (IS_HW_VERSION_SUPPORTS_ROTATION(ctrl)) {
