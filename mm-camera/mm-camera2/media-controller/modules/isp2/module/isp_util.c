  ...
  iface_fetch_engine_cfg_t *iface_fetch_engine_cfg)
{
  ...
  awb_meta = (cam_awb_params_t *)
    POINTER_OF_META(CAM_INTF_META_AWB_INFO, metadata);

  if (!awb_meta | !session_meta) {
    ISP_ERR("failed: NULL pointer %p %p",
      awb_meta, session_meta);
    return FALSE;
  }
  ...
  isp_session_param_t *session_param, mct_event_t *event, isp_hw_id_t hw_id)
{
  ...
  mct_stream_session_metadata_info *priv_metadata = NULL;
  ...
  session_param->fetch_eng_cfg_data.cds_enable = 0;
  priv_metadata = (mct_stream_session_metadata_info *)
    POINTER_OF_META(CAM_INTF_META_PRIVATE_DATA, metadata);
  if (!priv_metadata) {
    session_param->fetch_eng_cfg_data.cds_enable =
  ...
  isp_session_param_t *session_param, boolean is_online)
{
  ...
  if (is_online) {
    ...
    hw_update_list_params[session_param->hw_id[0]].applied_dmi_tbl;
  ...
  boolean is_online)
{
  ...
  pipeline_applying_delay = isp->isp_resource.isp_resource_info
    [session_param->hw_id[0]].isp_pipeline->pipeline_applying_delay;
  ...
  for (i = 0; i < ISP_MAX_STREAMS; i++) {
    ...
    ISP_DBG("ide %x scale ratio %f stream crop %d %d %d %d",
  ...
  isp_set_param_type_t *isp_type, uint32_t *set_parm_size)
{
  ...
  case CAM_INTF_PARM_LED_CALIBRATION:
    *isp_type = ISP_SET_LED_CALIBRATION;
    *set_parm_size = sizeof(uint32_t);
    break;
  ...
  isp_util_stream_desc *sorted_streams, cam_dimension_t *sensor_dim)
{
  ...
  for (i = 0; i < (int32_t)streams_desc->num_streams; i++) {
    ...
    sorted_streams[i].dim = streams_desc->stream_sizes[i];
          ...
          if (isp_util_compare_dim(dim1, dim2) &&
            ...
            for (j = (i -1); j > 0; j--) {
                fit_in_port = check_dim_fit_in_ratio(&sorted_streams[j].dim,
                  ...
                  PPROC_MAX_SCALERS_RATIO);
                if(!fit_in_port || IS_PP_MASK_FIT(&sorted_streams[j].dim,
                   &sorted_streams[j -1].dim)) {
                  additional_hw_stream_required++;
                }
            }
  ...
  isp_submod_hw_limit_t *isp_hw_limit)
{
  ...
  for (i = 0; i < priority_streams.stream_num; i++) {
    ISP_ERR("INFO: priority stream: %d num: %d mask: 0x%x", i,
      priority_streams.streams[i].max_streams_num,
      priority_streams.streams[i].stream_mask);
  }
...
/** isp_util_send_hw_link_to_internal_pipeline
 *    @session_param: handle to store session param
 *    @identity: identity
 *    @hw_id: array of ISP hw IDs
 *    @num_isp: num of ISPs
 ...
 * Return TRUE on success and FALSE if streams could not be mapped
 **/
boolean isp_util_send_hw_link_to_internal_pipeline(
  ...
  uint32_t num_isp)
{
  ...
  private_event.type = ISP_PRIVATE_HW_STREAM_LINKING;
  private_event.data = session_param->hw_stream_linked;
  ...
  uint32_t identity, isp_hw_id_t *hw_id, uint32_t num_isp)
{
  ...
  ret = isp_util_send_hw_link_to_internal_pipeline(session_param, identity,
    hw_id, num_isp);
  if (ret == FALSE) {
    ISP_ERR("failed: to send hw stream links");
    return ret;
  }
