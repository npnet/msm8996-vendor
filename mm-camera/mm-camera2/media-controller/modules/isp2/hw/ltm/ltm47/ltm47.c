  ...
  void *data)
{
  ...
  ltm47_interpolate_curve(chromatix_ltm_normal, chromatix_ltm_backlit, mod,
    ASD_algo_data->backlit_scene_detect.backlit_LTM.enable);
...
/** ltm47_attenuate_master_scale:
 *
 *  @chromatix_LTM: chromatix_LTM_type
 *  @mod: ltm module data
 *
 *  Weaken the master scale curve and pack to lut
 *
 * Return TRUE on success and FALSE on failure
 **/
static boolean ltm47_attenuate_master_scale(int *ms_cur, ltm47_t* mod,
  float exp_based_ratio)
{
  ...
  ISP_DBG("dark_str %f highlight_str %f", dark_str, highlight_str);
  for (i = 0; i < LTM_HW_LUT_SIZE + 1; i++) {
    if (mod->enable_adrc) {
      temp = mod->ltm_adrc_algo.adrc_unpacked_master_scale[i] *
        exp_based_ratio;
    } else {
      temp = mod->unpacked_master_scale[i] * exp_based_ratio;
    }
   /* weaken master scale */
    if(i < mod->dark_boost_thr_index) {
      ...
      temp = temp * highlight_str;
    }
    ms_cur[i] = (int)Round(temp);
  }
  ...
  float ratio)
{
  ...
  int          i = 0;
  float        scale_factor, ltm_strength;
  ...
  ISP_DBG("exp_based_ratio %f", ratio);
  mod->ratio = ratio;
  ...
  ltm47_attenuate_master_scale(master_scale, mod, ltm_strength);

  pack_tbl_to_lut_i32(master_scale, mod->hw_master_scale,1.0f);

  if (mod->enable_adrc) {
    pack_tbl_to_lut_i32(mod->ltm_adrc_algo.adrc_unpacked_shift_scale,
      mod->hw_shift_scale, scale_factor);
    pack_tbl_to_lut_i32(mod->ltm_adrc_algo.adrc_unpacked_master_curve,
      mod->hw_master_curve, 1.0f);
  } else {
    pack_tbl_to_lut_i32_shift(mod->unpacked_shift_scale, mod->hw_shift_scale,
      scale_factor);
  }
  return TRUE;
}
 ...
 * Return TRUE on success and FALSE on failure
 **/
boolean ltm47_setup_dark_bright_region_index(chromatix_LTM_type *ltm,
  ltm47_t *mod, int *master_scale)
{
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  for (i = 0; i < CURVE_ENTRY_NUM + 1; i++) {
    mod->ltm_adrc_algo.adrc_unpacked_master_curve[i] =
      Round(LINEAR_INTERPOLATION_LTM(mod->ltm_adrc_algo.adrc_master_curve[i],
       (float)ltm->reservedData.master_curve[i],
       mod->ltm_adrc_algo.adrc_ltm_mix_rate));

    mod->ltm_adrc_algo.adrc_unpacked_master_scale[i] =
      MAX(mod->ltm_adrc_algo.adrc_master_scale[i],
       mod->unpacked_master_scale[i] * mod->ltm_adrc_algo.adrc_ltm_mix_rate);

    mapped_idx = Round(CLAMP_LTM(i * drc_gain_nonlinear, 0 , 128));
    mod->ltm_adrc_algo.adrc_unpacked_shift_scale[i] = Round(
      (mod->unpacked_shift_scale[mapped_idx]));
  }

  /* Update the dark_boost_thr_index and highlight_sup_thr_index based on
     adrc based master scale curve */
  ltm47_setup_dark_bright_region_index(ltm, mod,
    mod->ltm_adrc_algo.adrc_unpacked_master_scale);
