 ...
 *  Return void
 **/
static void mesh_rolloff40_calc_flash_trigger(isp_sub_module_t *isp_sub_module,
  ...
  void *data)
{
  ...
  if (mesh_rolloff->cur_flash_mode == CAM_FLASH_MODE_ON ||
    ...
    led1_value_mainflash =
      mesh_rolloff->dual_led_setting.led1_high_setting;
    led2_value_mainflash =
      mesh_rolloff->dual_led_setting.led2_high_setting;
    if (led1_value_mainflash + led2_value_mainflash == 0 ) {
       ISP_ERR("%s: No entries in LED Flash table\n",__func__);
    } else {
       led_mainflash_ratio =
       (float)led1_value_mainflash/(led1_value_mainflash + led2_value_mainflash);
       tblLedFlash =
       &(mesh_rolloff->rolloff_tbls.rolloff_tableset[0]->left[ISP_ROLLOFF_LED_FLASH]);
       tblStrobeFlash =
       &(mesh_rolloff->rolloff_tbls.rolloff_tableset[0]->left[ISP_ROLLOFF_STROBE_FLASH]);
       tblLedFlashStored = &(mesh_rolloff->led_rolloff_stored);
       mesh_rolloff40_table_interpolate( tblLedFlashStored, tblStrobeFlash,tblLedFlash,
                                                               led_mainflash_ratio);
      }
  }
 ...
 *  Return void
 **/
static void mesh_rolloff40_normalize_table(isp_sub_module_t *isp_sub_module,
  void *mesh_in_mod)
{
  int                        i = 0, j = 0, k = 0, l = 0;
