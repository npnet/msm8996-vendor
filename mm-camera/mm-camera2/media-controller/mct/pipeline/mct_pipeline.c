...
static int64_t default_jpeg_stall_durations_front[] = {
  ...
  0.077 * SEC_TO_NS_FACTOR,  //1200 *1200
...
static int64_t default_jpeg_stall_durations_back[] = {
  ...
  0.099 * SEC_TO_NS_FACTOR,  //1.3MP UXGA
  0.091 * SEC_TO_NS_FACTOR,  //WXGA
  0.085 * SEC_TO_NS_FACTOR,  //1200 *1200
  0.080 * SEC_TO_NS_FACTOR,  //HD720
  0.075 * SEC_TO_NS_FACTOR,  //1MP XGA
  0.068 * SEC_TO_NS_FACTOR,  //SVGA
  0.067 * SEC_TO_NS_FACTOR,  //FWVGA
    ...
    case CAM_MAPPING_BUF_TYPE_SYNC_RELATED_SENSORS_BUF: {
      ...
      if (msg->ds_buf[i].buffer) {
        pipeline->related_sensors_sync_buf =
          (cam_sync_related_sensors_event_info_t *)msg->ds_buf[i].buffer;
      } else {
        ret= FALSE;
  ...
  boolean send_immediately)
{
  ...
  if (!meta_stream) {
    CLOGE(CAM_MCT_MODULE, "Couldn't find meta stream");
    return;
  }
  ...
  mct_event_control_type_t event_type, boolean send_immediately)
{
  ...
  for(current = 0; current < pipeline->parm_buf_count; current++) {
    ...
    } else if (event_parm.type ==  CAM_INTF_META_STREAM_INFO) {
...
boolean mct_pipeline_send_module_wakeup(mct_pipeline_t *pipeline)
{
  ...
  if (MCT_MODULE_STATE_ASLEEP == pipeline->module_hw_state) {
    event_data.type = MCT_EVENT_CONTROL_HW_WAKEUP;
    event_data.control_event_data = NULL;
    ret = mct_pipeline_pack_event(MCT_EVENT_CONTROL_CMD,
...
boolean mct_pipeline_send_module_sleep(mct_pipeline_t *pipeline)
{
  ...
  if ((MCT_MODULE_STATE_AWAKE == pipeline->module_hw_state) &&
    (FRM_REQ_MODE_CONTINUOUS != pipeline->wake_mode) &&
    (parm_stream->current_frame_id > parm_stream->hw_sleep_frame_id)) {
    ...
    if (pipeline->send_event && !(pipeline->frame_num_idx_list)) {
      CLOGE(CAM_MCT_MODULE, "Sending HW_SLEEP at frame id %d session =%d",
  ...
  mct_bus_msg_ctrl_request_frame_t *ctrl)
{
  ...
  pipeline->wake_mode = ctrl->req_mode;

  /*Sending HW Sleep/Awake command to AUX cam module only */
  if (pipeline->aux_cam) {
    if (ctrl->request_flag) {
      return mct_pipeline_send_module_wakeup(pipeline);
    } else {
      return mct_pipeline_send_module_sleep(pipeline);
    }
  }
  return ret;
}
...
static boolean mct_pipeline_fill_caps_isp(mct_pipeline_t *pipeline)
{
 ...
 bool entry_available;
  ...
  // if dual camera bokeh snapshot size select sensor dim which is less than vfe max
  // hal requested mode from def pic size table < sensor mode < vfe max
  for (i = 0; (i <  local_data->sensor_cap.dim_fps_table_count && i < SENSOR_MAX_RESOLUTION);
    i++) {
     if (local_data->sensor_cap.dim_fps_table[i].dim.width <= local_data->isp_cap.max_encoder_size.width) {
       hal_data->bokeh_snapshot_size.width = local_data->sensor_cap.dim_fps_table[i].dim.width;
       hal_data->bokeh_snapshot_size.height= local_data->sensor_cap.dim_fps_table[i].dim.height;
       break;
     }
  }
  for (i = 0;(i < (sizeof(default_picture_sizes) / sizeof(cam_dimension_t))
    && i < MAX_SIZES_CNT);  i++ ) {
     if (default_picture_sizes[i].width <= hal_data->bokeh_snapshot_size.width &&
       default_picture_sizes[i].height <= hal_data->bokeh_snapshot_size.height) {
       hal_data->bokeh_snapshot_size = default_picture_sizes[i];
       break;
     }
  }

  hal_data->max_encoder_size =
    local_data->isp_cap.max_encoder_size;
  ...
  mct_pipeline_t *pipeline)
{
  ...
  case CAM_PRIV_SYNC_RELATED_SENSORS:{
    ...
    if(related_cam_info != NULL) {
      ...
      if (related_cam_info->type == CAM_TYPE_AUX) {
