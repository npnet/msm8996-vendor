/**********************************************************************
  Copyright (c) 2011-2012, 2016, 2019 Qualcomm Technologies, Inc.  All Rights Reserved.
...
int aec_init_data(stats_proc_t *sproc, aec_t *aec)
{
  ...
  if(aec->exp_tbl_ptr ==  NULL){
    aec->exp_tbl_ptr = (exposure_entry_adapter *)malloc(MAX_EXPOSURE_TABLE_SIZE *
      sizeof(exposure_entry_adapter));
    if(!aec->exp_tbl_ptr) {
...
static void aec_load_exp_tbl(stats_proc_t *sproc, aec_t *aec)
{
  ...
  memcpy(aec->exp_tbl_ptr, sproc->input.chromatix->chromatix_exposure_table.
    ...
    valid_entries )* sizeof(exposure_entry_adapter));
