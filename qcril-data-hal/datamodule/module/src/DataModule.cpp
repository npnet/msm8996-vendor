...
void DataModule::handleDataEmbmsActiveMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL && call_manager != nullptr)
  {
    bool is_Embms_Active = false;
    if(call_manager != nullptr)
    {
      is_Embms_Active = call_manager->findEmbmsCallInfo();
    }
...
void DataModule::handleVoiceIndMessage(std::shared_ptr<Message> msg) {
  ...
  if(m != NULL)
  {
    ...
    if(m->getSubId() == SubId::SECONDARY && voiceCallEndPointSub1->mIsVoiceCallOrigTimer)
    {
     ...
     //Temporary switch
     if(((int)m->getSubId() != preferred_data_state_info->dds || preferred_data_state_info->switch_type == DSD_DDS_SWITCH_TEMPORARY_V01) && m->getCallState() !=  VoiceCallStateEnum::CALL_STATE_END)
     {
...
void DataModule::handleCurrentRoamingStatusChangedMessage(std::shared_ptr<Message> msg) {
  ...
  if (msg != nullptr) {
    ...
    Log::getInstance().d("CurrentRoamingStatusChangedMessage is NULL");
  }
}
