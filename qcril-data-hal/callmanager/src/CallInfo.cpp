...
CallInfo::CallInfo(LocalLogBuffer& setLogBuffer, GlobalCallInfo& setGlobalInfo):
    logBuffer(setLogBuffer), globalInfo(setGlobalInfo) {
...
bool CallInfo::setProfileInfo(CallSetupProfile *profile) {
  ...
  if((!pProfileInfo->getApn().empty()) || (pProfileInfo->getApn().empty() && DATA_IS_RIL_RADIO_TECH_3GPP(pProfileInfo->getAvailableRadioRAT()))) {
    ...
    if ((pProfileInfo->getCdmaProfileId() != QDP_PROFILE_ID_INVALID ||
        ...
        (pProfileInfo->lookup())) {
      /* Update the modem default profile in below 2 cases
          - When preferred flag is set for the profile received in setupdatcall
          - If preferred flag = 0 and there is not any other default call up */
      if((pProfileInfo->getApnTypes() & (uint32_t)ApnTypes_t::DEFAULT) && ((pProfileInfo->getPreferred())||(!pProfileInfo->getAnotherActiveDefaultCallPresent()))) {
...
dsi_ce_reason_t CallInfo::pickTemporaryCallEndReason() {
  dsi_ce_reason_t v4Reason = dsiGetVerboseCallEndReason(dsiHandle, "IP");
  DataCallFailCause_t v4RilReason = CallEndReason::getCallEndReason(v4Reason);
  if (!CallEndReason::isPermanentFailure(v4RilReason)) {
    return v4Reason;
  } else {
    dsi_ce_reason_t v6Reason = dsiGetVerboseCallEndReason(dsiHandle, "IPV6");
    return v6Reason;
  }
}

dsi_ce_reason_t CallInfo::getVerboseCallEndReason() {
  ...
  if (pProfileInfo != nullptr) {
    if (globalInfo.isRoaming) {
      currentIpType = getRequestedRoamingIpType();
    } else {
      currentIpType = getRequestedIpType();
    }
  }
  if (currentIpType == "IPV4V6") {
    dsiEndReason = pickTemporaryCallEndReason();
  } else {
    dsiEndReason = dsiGetVerboseCallEndReason(dsiHandle, currentIpType);
  }
  return dsiEndReason;
}
...
bool CallInfo::configureParams(bool updateDataCallListChangeInd) {
  ...
  CallInfo tempCallinfo (logBuffer, globalInfo);
...
void CallInfo::createQmiWdsClients() {
  ...
  if(this->v4Connected && wds_v4_call_endpoint == nullptr) {
    ...
    wds_v4_call_endpoint->setLinkStateChangeReport(globalInfo.linkStateChangeReport);
  }
  ...
  if(this->v6Connected && wds_v6_call_endpoint == nullptr) {
    ...
    wds_v6_call_endpoint->setLinkStateChangeReport(globalInfo.linkStateChangeReport);
  }
}
...
void CallInfo::updateLinkStateChangeReport()
{
  Log::getInstance().d("[CallInfo]: updateLinkStateChangeReport cid="+std::to_string((int)cId));
  if(wds_v4_call_endpoint) {
    wds_v4_call_endpoint->setLinkStateChangeReport(globalInfo.linkStateChangeReport);
  }
  if(wds_v6_call_endpoint) {
