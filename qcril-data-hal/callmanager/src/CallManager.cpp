...
bool CallManager::findDefaultCallInfo() {
  Log::getInstance().d("[CallManager]: findCallInfo with default apnType");

  std::set<CallInfo *>::iterator it;
  for (it=pCallInfoList.begin(); it!=pCallInfoList.end(); ++it) {
    if(((*it)->getProfileInfo() && (*it)->getProfileInfo()->getApnTypes()&(uint32_t)ApnTypes_t::DEFAULT))
    {
      Log::getInstance().d("[CallManager]: found callInfo");
      return true;
    }
  }
  return false;
...
bool CallManager::findEmbmsCallInfo() {
  Log::getInstance().d("[CallManager]: findEmbmsCallInfo");

  std::set<CallInfo *>::iterator it;
  for (it=pCallInfoList.begin(); it!=pCallInfoList.end(); ++it) {
    if((*it)->pEmbmsInfo != nullptr)
    {
      Log::getInstance().d("[CallManager]: found embmsCallInfo with cid " + std::to_string((*it)->getCid()));
      return true;
    }
  }
  return false;
...
void CallManager::handleSetupDataCallRequestMessage(std::shared_ptr<Message> msg) {
  ...
  if (m != nullptr) {
    ...
    pCallInstance = new CallInfo(logBuffer, globalInfo);
    ...
    pCallInstance->serialId = m->getSerial();
    pCallInstance->setMessageToken(token);
    pCallInstance->setRequestedIpType(m->getProtocol());
    pCallInstance->setRequestedRoamingIpType(m->getRoamingProtocol());
    ...
    /* Check whether any default call is present */
    if(profile != nullptr && !profile->getPreferred())
    {
      profile->setAnotherActiveDefaultCallPresent(findDefaultCallInfo());
    }
...
void CallManager::getRadioDataCallList(vector<DataCallResult_t> &call)
{
  ...
  for (it=pCallInfoList.begin(); it!=pCallInfoList.end(); ++it) {
    if((_eWWAN == ((*it)->getCurrentNetworkType() & _eWWAN)) ||
       ( !mApAssistMode && ( _eIWLAN == ((*it)->getCurrentNetworkType() & _eIWLAN)))) {
...
void CallManager::dataCallListChanged() {
  ...
  for (it=pCallInfoList.begin(); it!=pCallInfoList.end(); ++it) {
    ...
    if((!mApAssistMode && (HandoffNetworkType_t::_eIWLAN == ((*it)->getCurrentNetworkType() &
                                                         HandoffNetworkType_t::_eIWLAN))) ||
               (HandoffNetworkType_t::_eWWAN == ((*it)->getCurrentNetworkType() &
...
void CallManager::handleCallBringupFallbackMessage(std::shared_ptr<Message> msg)
{
  ...
  if(m != nullptr) {
    ...
    if (pCallInstanceOld != nullptr) {
      CallInfo *pCallInstanceNew = new CallInfo(logBuffer, globalInfo);
    ...
    if (pCallInstance != nullptr) {
      switch(evt.type) {
        ...
        if(pCallInstance->getActiveState()!=_eActivePhysicalLinkUp) {
          ...
          if (globalInfo.linkStateChangeReport) {
            pCallInstance->setCallParamsChangedFlag(true);
            dataCallListChanged();
          }
          else {
...
{
  Log::getInstance().d("[CallManager] : handleEmbmsEnableDataRequestMessage");
  CallInfo *pCallInstance = nullptr;
  pCallInstance = new CallInfo(logBuffer, globalInfo);
