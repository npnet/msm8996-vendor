...
class CallInfo {
public:
  CallInfo(LocalLogBuffer& logBuffer, GlobalCallInfo& globalInfo);
  ~CallInfo();
  ...
  string getRequestedIpType(void) {return requestedIpType;};

  string getRequestedRoamingIpType(void) {return requestedRoamingIpType;};

  int32_t getMtu(void) {return mtu;};

  timer_t getHandoverTimerId(void) {return handoverTimerId;};

  unsigned long getMaxPartialRetryTimeout(void) {return globalInfo.maxPartialRetryTimeout;};

  bool IsPartialRetryEnabled(void) {return globalInfo.partialRetryEnabled;};

  bool IsRoaming() { return globalInfo.isRoaming; }
  ...
  void setRequestedIpType(std::string ipType) { requestedIpType = ipType; }

  void setRequestedRoamingIpType(std::string ipType) { requestedRoamingIpType = ipType; }
  ...
  dsi_ce_reason_t pickTemporaryCallEndReason();
