    ...
    private void queryUiccProvisionInfo(int phoneId, boolean useSimIORequest) {
        ...
        if (mSimIccId[phoneId] == null || !mSimIccId[phoneId].equals(iccId)) {
            if (uiccCard != null) {
                ...
                if (uiccSlot != null) {
                    boolean eUiccSupported = uiccSlot.isEuicc();
                    if(uiccProfile != null) {
                        int numOfApps = uiccProfile.getNumApplications();
                        if (numOfApps == 0 && eUiccSupported) {
                            ...
                            if (!TextUtils.isEmpty(iccId)) {
                                ...
                                logd(" queryUiccProvisionInfo: useSimIORequest=  "
                                        + useSimIORequest);
                                if (useSimIORequest) {
                                    loadIccId(phoneId);
                                } else {
                                    ...
                                    // Once IccId avaiable add subInfo record entry
                                    if (!TextUtils.isEmpty(iccId)) {
                                        logi("OEM add subInfo record, iccId[" + phoneId + "] = "
                                                + iccId);
                                        mSimFullIccId[phoneId] = fullIccId;
                                        QtiSubscriptionInfoUpdater.getInstance().
                                                addSubInfoRecord(phoneId, iccId);
                                        ...
                                        logi(" queryUiccProvisionInfo, ICCID[" + phoneId +
                                                "] is null");
                                    }
                                }
                            }
