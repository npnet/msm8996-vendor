...
std::string PerfGlueLayer::PerfGlueLayerSyncRequest(int cmd) {
    pthread_mutex_lock(&mMutex);
    std::string res;
    for (int i = 0; i < MAX_MODULES; i++) {
        if (!mModules[i].IsEmpty()) {
            if (mModules[i].GetLib().SyncRequest) {
                const char* requestInfo = mModules[i].GetLib().SyncRequest(cmd);
                if (requestInfo) {
                    res.append(requestInfo);
                    res.append("\n");
                }
            }
        }
...
int PerfModule::LoadPerfLib(const char *libname) {
    ...
    if (!mLibHandle.is_opened && (NULL != libname)) {
         ...
         *(void **) (&mLibHandle.SyncRequest) =
             dlsym(mLibHandle.dlhandle, "perfmodule_sync_request");
         if ((rc = dlerror()) != NULL) {
             QLOGE("%s Failed to get perfmodule_sync_request\n", __func__);
         }
