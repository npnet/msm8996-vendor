...
void Target::InitializeTarget() {
    ...
    hintExt.Register(VENDOR_HINT_FPS_UPDATE, FpsUpdateAction::FpsUpdatePreAction, NULL);
...
void invoke_tinyu_wa(int socid) {
  ...
  /*for lito,atoll,lagoon we have only two gold cors, so mask is changed accrodingly*/
  if(socid == 400 || socid == 440 || socid == 407 || socid == 434 ||  socid == 459) {
