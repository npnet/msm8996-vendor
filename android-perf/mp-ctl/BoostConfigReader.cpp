...
PerfDataStore::BoostConfigInfo::BoostConfigInfo(int idnum, int type, bool enable, int timeout, int fps, char *tname, int res, char *resourcesPtr) {
    mId = idnum;
    mType = type;
    mEnable = enable;
    mTimeout = timeout;
    ...
    if(!xmlStrcmp(node->name, BAD_CAST BOOSTS_CONFIGS_XML_ELEM_CONFIG_TAG)) {
        ...
        if (valid_kernel && resourcesPtr != NULL) {
            ...
            if (idnum > VENDOR_HINT_START && idnum < VENDOR_PERF_HINT_END) {
                store->mBoostConfigs.push_back(new BoostConfigInfo(idnum, type, en, timeout, fps, tname, res, resourcesPtr));
            } else {
                //power hint
                store->mPowerHint.push_back(new BoostConfigInfo(idnum, type, en, timeout, fps, tname, res, resourcesPtr));
            }
